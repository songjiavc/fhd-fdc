...
@Service
public class OrganizationGraphDrawBO implements IGraphDrawBO {
	...
	public Map<String, Object> showStatus(String id) {
		...
		idList.add(id);
		List<Map<String,Object>> list = this.findLatestOrgAdjustHistoryByRiskIds(idList);
		if(list.size()>0){
	...
	public List<Map<String, Object>> showStatus(List<String> idList) {
		...
		for(Map<String,Object> map : list){
			...
			if(map.get("status")!=null){
				...
				String status = map.get("status").toString();
				if(status.equals(Contents.RISK_LEVEL_HIGH)){
					riskLevel = OrganizationGraphDrawBO.RISK_LEVEL_RED;
				}else if(status.equals(Contents.RISK_LEVEL_MIDDLE)){
					riskLevel = OrganizationGraphDrawBO.RISK_LEVEL_YELLOW;
				}else if(status.equals(Contents.RISK_LEVEL_LOW)){
					riskLevel = OrganizationGraphDrawBO.RISK_LEVEL_GREEN;
				}else{}
	...
	public List<Map<String,Object>> findLatestOrgAdjustHistoryByRiskIds(List<String> idList){
		List<Map<String,Object>> resultList = new ArrayList<Map<String,Object>>();
		...
		sql.append("SELECT h.id hid,org.id,org.org_name,h.assessement_status status,dict.dict_entry_value trend,h.risk_status score")
		.append(" FROM t_sys_organization org")
		.append(" LEFT JOIN  t_rm_org_adjust_history h on h.org_id = org.id and h.is_latest='1'")
		.append(" LEFT JOIN  t_sys_dict_entry dict on dict.id = h.etrend")
		.append(" WHERE org.id in (:ids)");
