...
@Service
public class ProcessGraphDrawBO implements IGraphDrawBO {
	...
	public Map<String, Object> showStatus(String id) {
		...
		idList.add(id);
		List<Map<String,Object>> list = this.findLatestProcessAdjustHistoryByRiskIds(idList);
		if(list.size()>0){
	...
	public List<Map<String, Object>> showStatus(List<String> idList) {
		...
		for(Map<String,Object> map : list){
			...
			if(map.get("status")!=null){
				...
				String status = map.get("status").toString();
				if(status.equals(Contents.RISK_LEVEL_HIGH)){
					riskLevel = ProcessGraphDrawBO.RISK_LEVEL_RED;
				}else if(status.equals(Contents.RISK_LEVEL_MIDDLE)){
					riskLevel = ProcessGraphDrawBO.RISK_LEVEL_YELLOW;
				}else if(status.equals(Contents.RISK_LEVEL_LOW)){
					riskLevel = ProcessGraphDrawBO.RISK_LEVEL_GREEN;
				}else{}
	...
	public List<Map<String,Object>> findLatestProcessAdjustHistoryByRiskIds(List<String> idList){
		List<Map<String,Object>> resultList = new ArrayList<Map<String,Object>>();
		...
		sql.append("SELECT h.id hid,p.id,p.processure_name name,h.assessement_status status,dict.dict_entry_value trend,h.risk_status score")
		.append(" FROM t_ic_processure p")
		.append(" LEFT JOIN  t_rm_processure_adjust_history h on h.processure_id = p.id and h.is_latest='1'")
