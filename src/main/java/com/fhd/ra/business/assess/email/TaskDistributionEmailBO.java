...
@Service
public class TaskDistributionEmailBO {
	 ...
	 * @param empId
	 */
	public void sendTaskDistributionEmail(String assessPlanId, Map<String, Long> empIdTaskIdMap){
		TemplateManage temp = o_templateManageBO.findDefaultTemplateByentryId("template_manage_assessplan_all");
	   	if(null!=temp){
	   		...
	   		String title = plan.getPlanName()+"　任务分配邮件";
			...
			while (iter.hasNext()) {
				...
				String taskId = value.toString();
	   			...
	   			String basePath = o_addRessUtilBO.getAddRessEmil(assessPlanId, taskId, "FHD.view.risk.assess.kpiSet.SetMainPanel", loginType, userName);
	   			String assessButton = "<a href=\"" + basePath + "\">立即分配<a/>";
				...
				List<RiskScoreObject> scoreObjectList = o_scoreObjectBO.findRiskScoreObjsByobjIdList(objIds);
		   		...
		   		for (RiskScoreObject riskScoreObject : scoreObjectList) {
		   			riskConten = riskConten.append("<tr><td><font color='red'>" + 
		   									riskScoreObject.getRisk().getName() + "</font></td></tr>");
				}
