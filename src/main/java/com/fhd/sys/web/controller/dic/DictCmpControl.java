...
public class DictCmpControl {
    ...
    public List<Map<String, String>> findDictEntryByTypeId(String typeId){
		...
		List<DictEntryI18> i18List = o_dictCmpBO.findDictEntryI18NameBySome(typeId, locale);
		...
		if(list != null && list.size() > 0){
			for(DictEntryForm dictEntryForm:list)
			{
			    ...
			    String tmpObj=null;
			    for (DictEntryI18 dictEntryI18 : i18List) {
					if(dictEntryI18.getDictEntry().getId().equals(dictEntryForm.getId())){
						tmpObj = dictEntryI18.getName();
					}
				}
