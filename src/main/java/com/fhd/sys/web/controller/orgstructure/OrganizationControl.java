...
public class OrganizationControl {
	...
	public void p_delete(String id,String parentid,HttpServletResponse response)throws Exception {
		...
		for(int i=0;i<ids.length;i++){
			if(StringUtils.isNotBlank(ids[i])){
				...
				//机构下有员工存在，不允许删除
				List<SysEmpOrg> empOrgs = o_sysEmpOrgBO.querySysEmpOrgByOrgid(orgid);
				if(!empOrgs.isEmpty()){
				...
				//查询出所有的机构和子机构
				List<SysOrganization> orgs = o_organizationBO.queryOrgsByOrgid(orgid);
				for(SysOrganization org : orgs){
					Set<SysPosition> positions = org.getSysPositions();
					for(SysPosition position : positions){
						//岗位下有员工存在，不允许删除
						List<SysEmpPosi> empPosis = o_sysEmpPosiBO.querySysEmpPosiByPosiId(position.getId());
						if(!empPosis.isEmpty()){
	...
	*/
	...
	public void uploadExcel(FileUploadBean fileUploadBean,
		...
		try {
			if (null != fileUploadBean && null != fileUploadBean.getUploadFile()) {
