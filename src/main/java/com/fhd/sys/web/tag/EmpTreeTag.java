 ...
 * @version  
 * @since    Ver 1.1
 * @Date	 2012-2-23		下午03:01:55
 *
 * @see 	 
 */
public class EmpTreeTag extends TagSupport {
	...
	private static final long serialVersionUID = 2244374444502168226L;
	...
	private String checkModel = "single";
	...
	private String sbEmptyText = "查找员工";//查询框默认文本
	...
	public int doStartTag() throws JspException {
		...
		try {
			BufferedReader br = new BufferedReader(new InputStreamReader(
				    ((EmpTreeTag.class.getResourceAsStream("/component_vm/SearchTreeTag.vm")))));
			...
			context.put("beforeload","var cls = node.attributes.cls;if('org' === cls)treeloader" + id + ".dataUrl = '"+  pageContext.getServletContext().getContextPath() + "/sys/orgstructure/org/loadPositionTree.do?leaf=emp';if('posi' === cls)treeloader" + id + ".dataUrl = '"+  pageContext.getServletContext().getContextPath() + "/sys/orgstructure/posi/loadPosiTree.do';");
			
			context.put("searchPath", pageContext.getServletContext().getContextPath() + "/sys/emp/getPathsBySearchName.do");
			
			if(StringUtils.isNotBlank(dataUrl)){
				context.put("dataUrl", dataUrl);
			}else{//树数据源地址
				context.put("dataUrl", pageContext.getServletContext().getContextPath() + "/sys/orgstructure/org/loadPositionTree.do?leaf=emp&empfilter=" + empfilter + "&defaultOrg=" + defaultOrg);
			}
