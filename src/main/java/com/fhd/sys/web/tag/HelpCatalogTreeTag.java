 ...
 * @version  
 * @since    Ver 1.1
 * @Date	 2012-7-23		下午03:11:16
 *
 * @see 	 
 */
public class HelpCatalogTreeTag extends TagSupport {
	...
	private static final long serialVersionUID = 4624824400691429244L;
	...
	private String sbEmptyText = "查找知识分类";//查询框默认文本
	...
	public int doStartTag() throws JspException {
		...
		try {
			BufferedReader br = new BufferedReader(new InputStreamReader(
				    ((HelpCatalogTreeTag.class.getResourceAsStream("/component_vm/SearchTreeTag.vm")))));
			...
			//context.put("searchPath", pageContext.getServletContext().getContextPath() + "/sys/duty/getPathsBySearchName.do");
			context.put("searchPath", pageContext.getServletContext().getContextPath() + "/sys/helponline/getPathsBySearchName.do");
			
			if(StringUtils.isNotBlank(dataUrl)){
				context.put("dataUrl", dataUrl);
			}else{//树数据源地址
				context.put("dataUrl", pageContext.getServletContext().getContextPath() + "/sys/helponline/loadTree.do");
//				context.put("dataUrl", pageContext.getServletContext().getContextPath() + "/sys/duty/loadTree.do");
			}

			StringBuilder baseParams = new StringBuilder("{");
			baseParams.append("checkNode:").append(checkNode).append(",choose:'").append("asas").append("'}");
