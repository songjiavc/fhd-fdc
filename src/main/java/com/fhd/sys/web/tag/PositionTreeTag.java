 ...
 * @version  
 * @since    Ver 1.1
 * @Date	 2012-2-23		下午03:40:17
 *
 * @see 	 
 */
public class PositionTreeTag extends TagSupport {
	...
	private static final long serialVersionUID = 552072413561269903L;
	...
	private String sbEmptyText = "查找岗位";//查询框默认文本
	...
	public int doStartTag() throws JspException {
		...
		try {
			BufferedReader br = new BufferedReader(new InputStreamReader(
				    ((PositionTreeTag.class.getResourceAsStream("/component_vm/SearchTreeTag.vm")))));
			...
			context.put("searchPath", pageContext.getServletContext().getContextPath() + "/sys/positon/getPathsBySearchName.do");
			
			if(StringUtils.isNotBlank(dataUrl)){
				context.put("dataUrl", dataUrl);
			}else{//树数据源地址
				context.put("dataUrl", pageContext.getServletContext().getContextPath() + "/sys/orgstructure/org/loadPositionTree.do?leaf=position");
			}
