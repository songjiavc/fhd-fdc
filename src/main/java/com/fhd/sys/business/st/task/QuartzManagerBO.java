...
public class QuartzManagerBO {
	...
	private Logger logger = Logger.getLogger(QuartzManagerBO.class);
	
	private static final String JOB_GROUP_NAME = "group1";//任务组
	private static final String TRIGGER_GROUP_NAME = "trigger1";//触发组
	 ...
	 * @throws Exception 
	 */	
	public void addJob(String jobName, Job job, String time) throws Exception {
		...
		try {
			...
			if(e.getMessage().indexOf("configured schedule, the given trigger will") != -1){
				logger.info("任务ID:" + jobName + "小于当前时间,任务永远不会触发");
			}else{
				logger.error("定时任务出错,原因未知.");
			}
			throw e;
		}
	}
	 ...
	 * @throws Exception 
	 */
	public void modifyJobTime(String jobName, Job job, String time) throws Exception {
         removeJob(jobName);
