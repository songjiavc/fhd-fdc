...
//		Page<SysUser> page = new Page<SysUser>();
	...
	public String  queryOrgByUserId(SysUser sysUser){
		String sysOrganizationName = "";
		SysEmployee sysEmployee = this.findEmpObjByUserId(sysUser);
		if(null != sysEmployee){
	 ...
	 */
	@SuppressWarnings("unchecked")
	public SysEmployee findEmpObjByUserId(SysUser sysUser){
		Criteria criteria = o_sysEmployeeDAO.createCriteria();
		criteria.add(Restrictions.eq("userid", sysUser.getId()));
		List<SysEmployee> sysEmployeeList = criteria.list();
		if(null != sysEmployeeList && !sysEmployeeList.isEmpty()){
			String sysEmployeeId = sysEmployeeList.get(0).getId();
			return this.querySysEmployeeById(sysEmployeeId);
		}
		return null;
	}
