...
import com.fhd.dao.sys.duty.DutyDAO;
...
import com.fhd.entity.sys.auth.SysRoleUser;
...
public class EmpGridBO implements IEmpGridBO{
	...
	@Transactional
	 public void removeEmpEntrys(String ids) {
		...
		for (String id : idArray) {
			...
			SysUser sysUser = empEntry.getSysUser();
			sysUser.setUserStatus(Contents.STATUS_DELETE);
			sysUser.setLockstate(true);
			sysUser.setEnable(false);
			empEntry.setDeleteStatus(Contents.DELETE_STATUS_DELETED);//逻辑删除
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<SysEmployee> findEmpEntryListByEmpIds(List<String> empIds) {
		Criteria c = o_sysEmployeeDAO.createCriteria();
		if (empIds.size()>0) {
			c.add(Restrictions.in("id", empIds));
		}
		return c.list();
	}
