...
import com.fhd.dao.sys.organization.SysPosiEmpDAO;
...
public class OrgPositionBO {
	...
	public List<SysPosition> findPositionByQueryOrIds(String posiName,String orgId) {
	 ...
	 public void removePositionByIds(String ids) {
		...
		for (String id : idArray) {
			...
			if(posiorg.getSysPositions().size()<=1){
				List<SysOrganization> orgList = o_orgGridBO.findChildOrgByparentId(posiorg.getId());
				if(null != orgList && orgList.size()>0){
					posiorg.setIsLeaf(false);
				}else{
	...
	public void addPosiEmp(String posiId,String empItem){
		List<String> exitEmpIds = this.findEmpidFromEmpPosiByPosiId(posiId);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<String> findEmpidFromEmpPosiByPosiId(String posiId){
		List<String> empIds = new ArrayList<String>();
		List<SysEmpPosi> list = null;
		Criteria c = o_sysPosiEmpDAO.createCriteria();
		if (StringUtils.isNotBlank(posiId)) {
			c.add(Restrictions.eq("sysPosition.id", posiId));
			list = c.list();
		}
		if(null != list){
			for(SysEmpPosi empPosi : list){
				empIds.add(empPosi.getSysEmployee().getId());
			}
		}
		return empIds;
	}
