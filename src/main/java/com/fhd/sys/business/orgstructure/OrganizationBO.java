...
public class OrganizationBO {
	...
	public SysOrganization findById(String id) {
		return o_organizationDAO.get(id);
	}
	/**
	 * 
	 * findByAll:
	 ...
	 * @since  fhd　Ver 1.1
	 */
	public List<SysOrganization> findByAll() {
		Criteria createCriteria = o_organizationDAO.createCriteria();
		createCriteria.add(Restrictions.eq("deleteStatus", Contents.DELETE_STATUS_USEFUL));
		return createCriteria.list();
	}
	/**
	 * 
	 * findByOrgcode:根据编号查询
	 * 
	 * @author 杨鹏
	 * @param orgcode
	 * @return
	 * @since  fhd　Ver 1.1
	 */
	public List<SysOrganization> findByOrgcode(String orgcode) {
		Criteria createCriteria = o_organizationDAO.createCriteria();
		createCriteria.add(Restrictions.eq("deleteStatus", Contents.DELETE_STATUS_USEFUL));
		createCriteria.add(Restrictions.eq("orgcode", orgcode));
		return createCriteria.list();
	}
	/**
	 * 
	 ...
	 * @since  fhd　Ver 1.1
	 */
	public List<SysOrganization> findByOrgcodes(String...orgcodes) {
		Criteria createCriteria = o_organizationDAO.createCriteria();
		createCriteria.add(Restrictions.eq("deleteStatus", Contents.DELETE_STATUS_USEFUL));
		createCriteria.add(Restrictions.in("orgcode", orgcodes));
		return createCriteria.list();
	}
	 ...
	 * @since  fhd　Ver 1.1
	 */
	public List<SysOrganization> findByName(String orgname) {
		Criteria createCriteria = o_organizationDAO.createCriteria();
		createCriteria.add(Restrictions.eq("orgname", orgname));
		createCriteria.add(Restrictions.eq("deleteStatus", Contents.DELETE_STATUS_USEFUL));
		createCriteria.add(Restrictions.eq("orgStatus", Contents.STATUS_NORMAL));
		return createCriteria.list();
	}
	/**
	 * 
	 ...
	 * @since  fhd　Ver 1.1
	 */
	public List<SysOrganization> findByCompanyId(String companyId) {
		Criteria createCriteria = o_organizationDAO.createCriteria();
		createCriteria.add(Restrictions.eq("company.id", companyId));
		createCriteria.add(Restrictions.eq("deleteStatus", Contents.DELETE_STATUS_USEFUL));
		createCriteria.add(Restrictions.eq("orgStatus", Contents.STATUS_NORMAL));
		return createCriteria.list();
	}
	 ...
	 * @since  fhd　Ver 1.1
	 */
	public List<SysOrganization> findBySome(String orgname,String companyId) {
		Criteria createCriteria = o_organizationDAO.createCriteria();
		createCriteria.add(Restrictions.eq("orgname", orgname));
		createCriteria.add(Restrictions.eq("company.id", companyId));
		createCriteria.add(Restrictions.eq("deleteStatus", Contents.DELETE_STATUS_USEFUL));
	...
	/**
	 * 
	 * findRootOrgByAll:查找组织根节点
	 * 
	 * @author 杨鹏
	 * @return
	 * @since  fhd　Ver 1.1
	 */
	public List<SysOrganization> findRootOrgByAll() {
		Criteria createCriteria = o_organizationDAO.createCriteria();
		createCriteria.add(Restrictions.isNull("parentOrg"));
		return createCriteria.list();
	}
	/**
	 * 
	...
	public void save(SysOrganization sysOrganization) {
		SysOrganization parentOrg = sysOrganization.getParentOrg();
		if(parentOrg!=null){
			parentOrg=this.findById(sysOrganization.getParentOrg().getId());
			if(parentOrg.getIsLeaf()){
				parentOrg.setIsLeaf(false);
				o_organizationDAO.merge(parentOrg);
			}
			sysOrganization.setOrgLevel(parentOrg.getOrgLevel() + 1);
			sysOrganization.setOrgseq(parentOrg.getOrgseq() + sysOrganization.getId() + ".");
			sysOrganization.setNameSeq(parentOrg.getNameSeq()+">" + sysOrganization.getOrgname());
		}else{
			sysOrganization.setOrgLevel(1);
			sysOrganization.setOrgseq("." + sysOrganization.getId() + ".");
			sysOrganization.setNameSeq(sysOrganization.getOrgname());
		}
		o_organizationDAO.merge(sysOrganization);
	}
...
//					node.put("href", contextPath + "/sys/orgstructure/emp/tabs.do?id=" + subEmp.getId());
	 ...
	 * @since  fhd　Ver 1.1
	 */
	public SysOrganization getSysOrgByCode(String orgCode){
		DetachedCriteria dc=DetachedCriteria.forClass(SysOrganization.class);
...
//		int sheetsum = workbook.getNumberOfSheets();
		...
		for (int i = emps.size() - 1; i >= 0; i--) {
			...
			// ||StringUtils.isBlank(emp.getCardtype())
				...
				if(StringUtils.isBlank(emp.getEmpStatus())){
					info.append("所属部门（如果是多个部门，以“，”分隔，默认第一个部门为主部门）为空!");
				}
				if(StringUtils.isBlank(emp.getCardno())){
					info.append("所属角色（如果是多个角色，以“，”分隔）为空!");
				}
				emp.setImpErrorInfo(info.append("信息填写不完整").toString());
			}
		}
		// 验证登录名
		for (int i = emps.size() - 1; i >= 0; i--) {
		...
		for (int i = emps.size() - 1; i >= 0; i--) {
			...
			for (String orgName : orgs) {
				...
				if (depCache.get(orgName+emp.getRealname()) != null) {
					...
					info.append("部门 '").append(orgName).append("'不存在!");
				}
			}
			if (info.length() > 0) {
				bads.add(emps.remove(i));
				emp.setImpErrorInfo(info.toString());
			}
		}
			...
			for (int index=0;index<posis.length;index++) {
				...
				if(orgs.length==1){//多个部门 分析为一一对应，一个部门 分析为一个部门多个岗位
					...
					if (psoiCache.get(emp.getRealname()+orgs[0]+posis[index]) != null) {
						...
						info.append("岗位'").append(posis[index]).append("'不存在!");
					}
				}else{//多个部门 分析为一一对应，
		...
		for (int i = emps.size() - 1; i >= 0; i--) {
			...
			for (String roleName : roles) {
				...
				if (roleCache.get(roleName) != null){
					...
					info.append("角色'").append(roleName).append("'不存在!");
				}
				if (!find) {
		...
		for (int i = dutys.size() - 1; i >= 0; i--) {
			...
			if (StringUtils.isBlank(duty.getDutyName()) || StringUtils.isBlank(duty.getRemark()) || StringUtils.isBlank(duty.getId())) {
				...
				if(StringUtils.isBlank(duty.getId())){
					info.append("所属公司名称为空!");
				}
				duty.setImpErrorInfo(info.append("必填项不完整!").toString());
			}
		}
		// 验证权限数字
		for (int i = dutys.size() - 1; i >= 0; i--) {
	...
	private List[] checkSysPosition(List<SysPosition> positions) {
		...
		for (int i = positions.size() - 1; i >= 0; i--) {
			...
			if (StringUtils.isBlank(pos.getPosicode()) || StringUtils.isBlank(pos.getPosiname()) || StringUtils.isBlank(pos.getPosiStatus()) || StringUtils.isBlank(pos.getRemark())) {
				...
				pos.setImpErrorInfo(info.append("信息填写不完整").toString());
			}
		}
		...
		for (int i = orgs.size() - 1; i >= 0; i--) {
			...
			if (StringUtils.isBlank(org.getOrgcode()) || StringUtils.isBlank(org.getOrgname()) || StringUtils.isBlank(org.getOrgType())) {
				...
				if(StringUtils.isBlank(org.getOrgType())){
					info.append("机构类型（总公司、总公司部门、分公司、分公司部门）为空!");
				}
				
				org.setImpErrorInfo(info.append("必填项不完整!").toString());
	...
	private boolean saveOrg(SysOrganization org) {
		if (null != org.getParentOrg()&& !saveOrg(org.getParentOrg())){
			return false;
		}
