...
import com.fhd.dao.sys.group.GroupDAO;
...
public class GroupBO {
	...
	public List<Map<String, Object>> getAuthority(String id) {
		...
		if ("rootId".equals(id)) {
			...
			auth = (SysAuthority) this.o_sysAuthorityDAOold.findByCriteria(dc).get(0);
		} else {
			auth = this.o_sysAuthorityDAOold.get(id);
		}
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		for (SysAuthority a : auth.getChildren()) {
			Map<String, Object> m = new HashMap<String, Object>();
			m.put("id", a.getId());
			m.put("text", a.getAuthorityName());
			m.put("leaf", a.getChildren().size() == 0);
			m.put("checked", false);
			list.add(m);
		}
		return list;
	...
	public void saveAuthority(String id, String ids) {
		if (ids != null) {
			...
			String[] arr = ids.split(",");
			for (String tid : arr) {
				SysAuthority au = this.o_sysAuthorityDAOold.get(tid);
				if (au != null) {
