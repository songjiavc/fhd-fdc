...
public class DictBO implements IDictBO{
	...
	*/
	...
	/**
	 * 根据DictEntry 名称查询 DictEntry
	 * @author 王鑫
	 * @param name 数据字典名称
	 * @return
	 */
	
	public String findDictEntryByName(String name){
		Criteria criteria = o_dictEntryDAO.createCriteria();
		if(null!=name){
	...
	/**
	 * @author 王鑫
	 * @return 数据字典名称/ID映射集合
	 */
    public Map<String, String> findDictEntryMap(){
     ...
     * @since  fhd　Ver 1.1
     */
	public DictEntry getBySome(String value,String dicTypeId){
		Criteria criteria = o_dictEntryDAO.createCriteria();
		criteria.add(Restrictions.ilike("value", value));
		criteria.add(Restrictions.eq("dictType.id", dicTypeId));
		return (DictEntry) criteria.uniqueResult();
	}
	
	/**
	 * 根据数据字典项id查找子数据
	 * add by 王再冉
	 * 2013-12-26  上午11:00:02
	 * desc : 
	 * @param dictEntryId
	 * @return 
	 * DictEntry
	 */
	@SuppressWarnings("unchecked")
	public List<DictEntry> findChildrenDictEntrysById(String dictEntryId) {
		Criteria criteria = o_dictEntryDAO.createCriteria();
		List<DictEntry> list = null;
		if (StringUtils.isNotBlank(dictEntryId)) {
			criteria.add(Restrictions.eq("parent.id", dictEntryId));
			list = criteria.list();
		} 
		return list;
    }
