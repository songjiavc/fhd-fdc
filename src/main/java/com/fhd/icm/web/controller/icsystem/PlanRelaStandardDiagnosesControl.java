...
public class PlanRelaStandardDiagnosesControl {
			...
			for(String diagnosesId : diagnosesIdArray){
				...
				for(Iterator<StandardRelaOrg> it = orgList.iterator();it.hasNext();){
					...
					if(Contents.ORG_RESPONSIBILITY.equals(org.getType())){
						diagnosesMap.put("standardRelaOrg",org.getOrg().getOrgname());//整改责任部门
					}
				}
				//根据计划标准id找到对应的责任人
	 ...
	 */
	@ResponseBody
	@RequestMapping("/icm/icsystem/getinitparamsfordefect.f")
	public Map<String, Object> findPlanRelaStandardDiagnosesListByPlanRelaStandardId(String planRelaStandardId) {
		Map<String, Object> map = new HashMap<String, Object>();
		DiagnosesRelaDefect diagnosesRelaDefect = o_planRelaStandardDiagnosesBO.findDiagnosesByPlanRelaStandardId(planRelaStandardId);
		if(null != diagnosesRelaDefect){			
			map.put("diagnosesRelaDefectId", diagnosesRelaDefect.getId());
			map.put("defectId", diagnosesRelaDefect.getDefect().getId());
		}
		return map;
	}
	...
	public Map<String, Object> findDefectIdbyConstructId(String constructPlanId,String executionId) {
		return o_planRelaStandardDiagnosesBO.findDefectIdbyExecutionId(executionId);
	}
	/**
	 * <pre>
	 * 根据风险id获取控制id
	 ...
	 */
	@ResponseBody
	@RequestMapping("/icm/icsystem/loaddefectclearupviewformdata.f")
	public Map<String, Object> loadDefectClearUpViewFormData(String defectId,String diagnosesId) {
		Map<String, Object> processRiskMap = o_planRelaStandardDiagnosesBO
				.loadDefectClearUpViewFormData(defectId,diagnosesId);
		return processRiskMap;
