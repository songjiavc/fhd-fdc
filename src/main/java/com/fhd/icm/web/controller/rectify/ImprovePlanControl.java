...
import com.fhd.entity.icm.rectify.DefectTrace;
	...
	*/
	...
	public Map<String, Object> findImprovePlanListByImproveId(int limit, int start,String improveId) {
		...
		for (DefectRelaImprove defectRelaImprove : defectRelaImproveList) {
			...
			for (ImprovePlanRelaDefect improvePlanRelaDefect : improvePlanRelaDefectTempSet) {//通过缺陷关联整改方案得到缺陷关联的整改计划
				for (ImproveRelaPlan improveRelaPlan : improveRelaPlanList) {//虚幻整改计划关联整改方案
					Set<DefectRelaOrg> defectRelaOrgSet = improvePlanRelaDefect.getDefect().getDefectRelaOrg();
					String onlineDeptId = UserContext.getUser().getMajorDeptId();//利用当前登录人的部门过滤整改方案和缺陷的数据
					String orgId = null;
