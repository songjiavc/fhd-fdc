...
public class ImproveBpmControl {
	...
	public void rectifyPublish(String executionId, String businessId,HttpServletResponse response) throws IOException{
		...
		try {
			...
			if(null != improve){
				...
				String iCDepartmentMinisterEmpId1 = o_improveBpmBO.findRectifyEmpIdByRole("ICDepartmentMinister");//内控部门部长
				if(null != empIds &&empIds.length>0 && StringUtils.isNotBlank(iCDepartmentMinisterEmpId1)){
					...
					for (DefectRelaImprove defectRelaImprove : defectRelaImproveList) {//DefectRelaImprove关联方案
					...
					if(null != improvePlanIdSet && improvePlanIdSet.size() == 0){//如果该缺陷没有关联整改方案，则需要创建
	...
	*/
	...
	public void rectifyPlanCheck(String executionId, String businessId, String examineApproveIdea,HttpServletResponse response) throws IOException{
        PrintWriter out = null;
		try {
			out = response.getWriter();
			Map<String, Object> variables=new HashMap<String, Object>();
			//将意见保存起来
			//variables.put("path", true);
			//variables.put("examineApproveIdea", "同意");//工作流中没有判断，直接写死
			o_jbpmBO.doProcessInstance(executionId, variables);
			out.write("true");
		}finally{
			if(null != out){
