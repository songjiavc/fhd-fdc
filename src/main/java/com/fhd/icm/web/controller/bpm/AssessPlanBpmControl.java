...
public class AssessPlanBpmControl {
	...
	*/
	 ...
	 * @param response
	 */
	@RequestMapping("/icm/assess/assessDefectSummaryStop.f")
	public void assessDefectSummaryStop(String executionId, String businessId, HttpServletResponse response) {
		PrintWriter out = null;
		try {
			Map<String, Object> variables = new HashMap<String, Object>();
			variables.put("end", "yes");
			o_jbpmBO.doProcessInstance(executionId, variables);
			out = response.getWriter();
			out.write("true");
		}catch (Exception e) {
	...
	public List<String> findEmpIdListByBusinessId(String businessId, String roleName){
			...
			for (AssessRelaDefect assessRelaDefect : assessRelaDefectList) {
				...
				if(null != defect){
					...
					for (DefectRelaOrg defectRelaOrg : defectRelaOrgs) {
						if(null != defectRelaOrg.getOrg() && !orgIdSet.contains(defectRelaOrg.getOrg().getId())){
		...
		if(null != objectList && objectList.size()>0){
			...
			for (Object[] objects : objectList) {
				if(null != objects[0] && !empIdList.contains(objects[0])){
