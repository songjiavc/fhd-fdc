...
public class StandardBpmBO implements IStandardBO {
	private static final Log LOG = LogFactory.getLog(StandardBpmBO.class);
	...
	*/
	public StandardBpmObject findStandardBpmObjectByExecutionId(String executionId){
		return (StandardBpmObject) o_jbpmOperate.getVariableObj(executionId, "item");
	}
	...
	*/
	public void startSubCompanyStandardApply(String standardId, Set<String> subCompanyIdList) throws IllegalAccessException, InvocationTargetException {
		...
		String companyId = standard.getCompany().getId();//得到内控标注对应公司ID
	...
	*/
	...
	public void startCurCompanyStandardApplyStepTwo(String executionId, String businessId, Standard standard,
			String[] standardControlIds, String isPass, String examineApproveIdea, String isTerminal){
		if("yes".equals(isTerminal)){//审批通过
			...
			if(null != standardBpmObjList && standardBpmObjList.size()>0){
				...
				LOG.error("流程获取内控标准分发时没有得到指定的人",new Exception("流程获取内控标准分发时没有得到指定的人"));
			}
		}else if("no".equals(isTerminal)){//终止流程，直接结束
			Map<String, Object> variables = new HashMap<String, Object>();
			variables.put("path", "end");
			o_jbpmBO.doProcessInstance(executionId, variables);
			//逻辑删除内控标准及下挂要求
			standard.setDeleteStatus(Contents.DELETE_STATUS_DELETED);
			o_standardDAO.merge(standard);
			for(String sid : standardControlIds){
				Standard standardControl = o_standardDAO.get(sid);
