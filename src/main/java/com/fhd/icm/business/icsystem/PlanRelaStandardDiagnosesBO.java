...
import com.fhd.dao.process.ProcessRelaStandardDAO;
...
import com.fhd.entity.process.ProcessRelaStandard;
...
public class PlanRelaStandardDiagnosesBO {
	...
	protected ConstructPlanRelaStandardEmp getConstructPlanRelaStandardEmpEntryById(String standardId,String empId){
	...
	public ConstructPlanRelaStandardEmp getPlanStandardEmp(String planStandardId){
		...
		return (ConstructPlanRelaStandardEmp) cri.uniqueResult();
	}
	...
	*/
	 ...
	 * @since  fhd　Ver 1.1
	 */
	public DiagnosesRelaDefect findDiagnosesByPlanRelaStandardId(String planRelaStandardId){
		Criteria criteria = o_diagnosesRelaDefectDAO.createCriteria();
		criteria.createAlias("diagnoses", "diagnoses");
		criteria.add(Restrictions.eq("diagnoses.constructPlanRelaStandard.id", planRelaStandardId));
		return (DiagnosesRelaDefect) criteria.uniqueResult();
	}
	...
	*/
	...
	public ProcessRelaStandard getProcessRelaStandard(String standardId){
		...
		return (ProcessRelaStandard) cri.uniqueResult();
	}
...
//		constructPlanRelaStandardMap.put("orgName", StringUtils.join(orgNameSet, ","));
		...
		for(Iterator<DefectRelaOrg> it = defectorgList.iterator();it.hasNext();){
			...
			if(Contents.ORG_RESPONSIBILITY.equals(org.getType())){
				...
				SysOrganization sysOrg = org.getOrg();
				obj.put("id", sysOrg.getId());
				obj.put("deptno", sysOrg.getOrgcode());
				obj.put("deptname", sysOrg.getOrgname());
				deptArr.add(obj);
			}
		}
		...
		if(defect.getRisk()!=null){
			riskMap.put("riskId", defect.getRisk().getId());
			riskMap.put("riskName", defect.getRisk().getName());
			formMap.put("riskId", riskMap);
		}
		Map<String, Object> node=new HashMap<String, Object>();
		node.put("data", formMap);
		node.put("success", true);
	 ...
	 * @since  fhd　Ver 1.1
	 */
	public Map<String,Object> loadDefectClearUpViewFormData(String defectId,String diagnosesId){
		Map<String, Object> formMap = new HashMap<String, Object>();
	...
	//		constructPlanRelaStandardMap.put("orgName", StringUtils.join(orgNameSet, ","));
			...
			Diagnoses diagnoses = o_diagnosesDAO.get(diagnosesId);
			Standard standard = diagnoses.getConstructPlanRelaStandard().getStandard();
			ProcessRelaStandard processRelaStandard = o_constructPlanRelaStandardBO.getProcessRelaStandard(standard.getId());
			//		constructPlanRelaStandardMap.put("orgName", StringUtils.join(orgNameSet, ","));
			if (null != processRelaStandard) {// 显示末级流程名称
						formMap.put("processName",processRelaStandard.getProcess().getName());
	...
	public void saveDefectClearUp(DefectClearUpGroupForm groupForm){
		...
		if(msForm != null){   //如果没有控制节点就只保存风险
			for(int i = 0 ;i < msForm.size(); i++){
				...
				if(jsonObject.has("riskId")){
					defect.setRisk(new Risk(jsonObject.getString("riskId")));
				}else{
					defect.setRisk(null);
				}
