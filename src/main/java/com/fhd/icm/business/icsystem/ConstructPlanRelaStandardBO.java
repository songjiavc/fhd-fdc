...
public class ConstructPlanRelaStandardBO {
	...
	/**
	 * 根据计划关联标准id 找到唯一的合规诊断id
	 * @param jsonString
	 * @param defaultValue
	 */
	public Diagnoses getDiagnosesByPlanRelaStandardId(String planRelaStandardId){
		Criteria criteria = o_diagnosesDAO.createCriteria();
		criteria.add(Restrictions.eq("constructPlanRelaStandard.id", planRelaStandardId));
		return (Diagnoses)criteria.uniqueResult();
	}
	/**
	 * 体系保存建设范围
	 * @param jsonString    id是AssessPlanRelaProcess的ID
	 * @param defaultValue  批量设置的建设人planRelaStandardEmpId的ID
	 */
	@Transactional
...
//	}
	...
	*/
	...
	public ProcessRelaStandard getProcessRelaStandard(String standardId){
		...
		cri.createAlias("process", "process");
		cri.add(Restrictions.eq("process.deleteStatus", Contents.DELETE_STATUS_USEFUL));
