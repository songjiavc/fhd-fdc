...
import com.fhd.dao.icm.assess.AssessFrequenceDAO;
import com.fhd.dao.icm.assess.AssessPlanDAO;
...
public class AssessPlanBO implements IAssessPlanBO {
	...
	private void saveAssessPlanRelaOrgEmp(String assessPlanId, String orgOrEmpId, String type) {
		...
		if (Contents.ORG_PARTICIPATION.equals(type) && StringUtils.isNotBlank(orgOrEmpId)){
			// 设置保存部门
			SysOrganization sysOrganization = new SysOrganization();
			sysOrganization.setId(orgOrEmpId);
			assessPlanRelaOrgEmg.setOrg(sysOrganization);
		} else if ((Contents.EMP_RESPONSIBILITY.equals(type) || Contents.EMP_HANDLER.equals(type)) && StringUtils.isNotBlank(orgOrEmpId)){
			// 设置保存组长或组员
						...
						for (AssessPlanRelaProcess assessPlanRelaProcess : assessPlanRelaProcesses) {
							...
							if(null != process){
								...
								for (ProcessRelaOrg processRelaOrg : processRelaOrgs) {
									if(StringUtils.isNotBlank(processRelaOrg.getType()) && Contents.EMP_RESPONSIBILITY.equals(processRelaOrg.getType()) && null != processRelaOrg.getEmp()){
	...
	public void mergeAssessPlanProcessureByAssessPlanId(AssessPlan assessPlan, String assessMeasureId){
		if(null != assessPlan && StringUtils.isNotBlank(assessMeasureId)){
	...
	public void removeAssessPlanRelaOrgEmpByAssessPlanId(String assessPlanId, String type) {
		...
		if(!list.isEmpty()){
			StringBuilder assessPlanRelaOrgEmpIds = new StringBuilder();
			int i=0;
			for (AssessPlanRelaOrgEmp assessPlanRelaOrgEmp : list) {
				assessPlanRelaOrgEmpIds.append("'").append(assessPlanRelaOrgEmp.getId()).append("'");
				if(i != list.size()-1){
					assessPlanRelaOrgEmpIds.append(",");
				}
			}
			//删除评价计划流程部门人员关联表数据
			o_assessPlanProcessRelaOrgEmpDAO.createQuery("delete AssessPlanProcessRelaOrgEmp appo where appo.assessPlanRelaOrgEmp.id in (:assessPlanRelaOrgEmpId)").setString("assessPlanRelaOrgEmpId", assessPlanRelaOrgEmpIds.toString()).executeUpdate();
		}
			...
			if(null!=processRelaOrg.getType()){
				if (Contents.EMP_RESPONSIBILITY.equals(processRelaOrg.getType()) && null != processRelaOrg.getEmp()){
					...
					for(Object[] empObject : empObjectList){
						if(null != empObject[0] && processRelaOrg.getEmp().getId().equals(empObject[0]) && Integer.valueOf(String.valueOf(empObject[1])) == 1){
							empSb.append(processRelaOrg.getEmp().getId());
							empSb.append(",");
						}
					}
				}else if (Contents.ORG_RESPONSIBILITY.equals(processRelaOrg.getType()) && null != processRelaOrg.getOrg()){
					// 参与部门
	...
	public void removeAssessPlanByIdList(List<String> idList) {
		if (!idList.isEmpty()) {
			...
			hql.append("update AssessPlan ap set ap.deleteStatus=:deleteStatus where ap.id in('").append(StringUtils.join(idList, "','")).append("')");
	 ...
	 * @param code
	 * @param name
	 * @return List<AssessPlan>
	 * @since fhd　Ver 1.1
	 */
	public List<AssessPlan> findAssessPlandBySome(String code, String name) {
	...
	public Double findFrequenceNumByProcess(Process process) {
		String riskAssementStatus = o_riskOutsideBO.findRiskAssementStatusByProcessId(process.getId());
		
		List<AssessFrequence> assessFrequenceList = findAssessFrequenceBySome();
		Double frequenceNum = 0.0;// 抽样比例
		for (AssessFrequence assessFrequence : assessFrequenceList) {
			if(null!=process.getFrequency() && assessFrequence.getCode().equals(process.getFrequency().getId())) {
				/*
				 * 这里需要匹配AssessFrequence表里的标准数据，然后根据当前流程的发生频率及相关的风险水平的高中低，来获得抽样比例，来获取需要抽取的样本数
				 */
				if("icon-ibm-symbol-4-sm".equals(riskAssementStatus)){
					//流程的风险水平为高
					frequenceNum = assessFrequence.getAmountMax();
				}else if("icon-ibm-symbol-5-sm".equals(riskAssementStatus)){
					//流程的风险水平为中
					frequenceNum = assessFrequence.getAmountMiddle();
				}else {
					frequenceNum = assessFrequence.getAmountMin();
				}
				break;
			}
		}
	...
	/**
	 * 根据processIdSet查询所有对应的流程风险状态.
	 * @author 吴德福
	 * @param processIds
	 * @return Map<String, Double>
	 */
	public Map<String, Double> findFrequenceNumMapByProcessList(String processIds) {
		Map<String, Double> map = new HashMap<String, Double>();
		
		List<Process> processList = o_processBO.findProcessByIds(processIds);
		List<ProcessAdjustHistory> processAdjustHistoryList = o_riskOutsideBO.findRiskAssementStatusByProcessIds(processIds);
		List<AssessFrequence> assessFrequenceList = findAssessFrequenceBySome();
		Double frequenceNum = 0.0;// 抽样比例
		
		for (ProcessAdjustHistory processAdjustHistory : processAdjustHistoryList) {
			for (AssessFrequence assessFrequence : assessFrequenceList) {
				if(null!=processAdjustHistory.getProcess().getFrequency() && assessFrequence.getCode().equals(processAdjustHistory.getProcess().getFrequency().getId())) {
					/*
					 * 这里需要匹配AssessFrequence表里的标准数据，然后根据当前流程的发生频率及相关的风险水平的高中低，来获得抽样比例，来获取需要抽取的样本数
					 */
					if("icon-ibm-symbol-4-sm".equals(processAdjustHistory.getAssessementStatus())){
						//流程的风险水平为高
						frequenceNum = assessFrequence.getAmountMax();
					}else if("icon-ibm-symbol-5-sm".equals(processAdjustHistory.getAssessementStatus())){
						//流程的风险水平为中
						frequenceNum = assessFrequence.getAmountMiddle();
					}else{
						//流程的风险水平为空，不操作--icon-ibm-symbol-0-sm  //默认值
						frequenceNum = assessFrequence.getAmountMin();
					}
					map.put(processAdjustHistory.getProcess().getId(), frequenceNum);
				}
			}
		}
		...
		for (Process process : processList) {
			if(!map.containsKey(process.getId())){
				for (AssessFrequence assessFrequence : assessFrequenceList) {
					if(null!=process.getFrequency() && assessFrequence.getCode().equals(process.getFrequency().getId())) {
						map.put(process.getId(), assessFrequence.getAmountMin());
					}
				}
			}
		}
	...
	*/
	public List<AssessPlanBpmObject> findAssessPlanRelaProcessParameterForBpmTask(String assessPlanId){
		...
		if(!assessPlanRelaProcessList.isEmpty()){
			...
			for(AssessPlanRelaProcess assessPlanRelaProcess : assessPlanRelaProcessList){
				...
				for(AssessPlanProcessRelaOrgEmp assessPlanProcessRelaOrgEmp : assessPlanProcessRelaOrgEmps){
					if(Contents.EMP_HANDLER.equals(assessPlanProcessRelaOrgEmp.getType()) && null !=assessPlanProcessRelaOrgEmp.getAssessPlanRelaOrgEmp() && null != assessPlanProcessRelaOrgEmp.getAssessPlanRelaOrgEmp().getEmp()){
						//评价计划对应的评价人
						assessPlanBpmObject.setExecuteEmpId(assessPlanProcessRelaOrgEmp.getAssessPlanRelaOrgEmp().getEmp().getId());
					}
					if(Contents.EMP_REVIEW_PERSON.equals(assessPlanProcessRelaOrgEmp.getType()) && null !=assessPlanProcessRelaOrgEmp.getAssessPlanRelaOrgEmp() && null != assessPlanProcessRelaOrgEmp.getAssessPlanRelaOrgEmp().getEmp()){
						//评价计划对应的复核人
