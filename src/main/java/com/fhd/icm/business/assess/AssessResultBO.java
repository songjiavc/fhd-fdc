...
import com.fhd.dao.icm.assess.AssessRelaDefectDAO;
...
import com.fhd.dao.icm.assess.AssessorDAO;
import com.fhd.dao.icm.defect.DefectDAO;
import com.fhd.dao.icm.defect.DefectRelaOrgDAO;
...
import com.fhd.entity.icm.assess.AssessPlanRelaPoint;
...
import com.fhd.entity.icm.assess.AssessSample;
import com.fhd.entity.icm.assess.AssessSampleRelaFile;
...
public class AssessResultBO {
	 ...
	 * 
	 * @author 黄晨曦
	 * @param isDesirableAdjust 不适用调整
	 * @param assessmentDesc	调整说明
	...
	public void saveAssessSampleBatch(String assessPlanId, String assessResultId, List<Integer> sampleNum, String sampletext){
		...
		for (int i = 0; i < sampleNum.size(); i++) {
			...
			if(StringUtils.isNotBlank(sampletext)){
				...
				assessSample.setCode("ZD"+sampleNum.get(i));
			}
	...
	*/
	 ...
	 * 根据样本id集合查询对应的补充样本列表.
	 * @author 吴德福
	 * @param assessSampleIdList
	 * @return Map<String,Integer>
	 */
	public Map<String,Integer> findSupplementAssessSampleMapByAssessSampleIdList(List<String> assessSampleIdList){
		Map<String,Integer> map = new HashMap<String,Integer>();
		Criteria criteria = o_assessSampleDAO.createCriteria();
		criteria.add(Restrictions.eq("type", Contents.SAMPLE_STATUS_SUPPLEMENT));
		if(!assessSampleIdList.isEmpty()){
			criteria.add(Restrictions.in("sourceSample.id", assessSampleIdList));
		}
		criteria.addOrder(Order.asc("sourceSample.id"));
		List<AssessSample> assessSampleList = criteria.list();
		int count=0;
		String tempAssessSampleId = "";
		int i=0;
		for (AssessSample assessSample : assessSampleList) {
			if(!"".equals(tempAssessSampleId)){
				if(tempAssessSampleId.equals(assessSample.getSourceSample().getId())){
					count++;
					if(i == assessSampleList.size()-1){
						map.put(tempAssessSampleId, count);
					}
				}else{
					map.put(tempAssessSampleId, count);
					tempAssessSampleId = assessSample.getSourceSample().getId();
					count = 1;
				}
			}else{
				tempAssessSampleId = assessSample.getSourceSample().getId();
	...
	*/
	...
	public void mergeAssessSampleBatch(String assessResultId, String jsonString){
		...
		for(int i=0;i<jsonArray.size();i++){
			...
			String isQualified = jsonObject.getString("isQualified");//是否有效
			...
			String impact = jsonObject.getString("impact");//说明
			for (AssessSample assessSample : assessSampleList) {
				if(StringUtils.isNotBlank(assessSampleId) && assessSampleId.equals(assessSample.getId())){
					...
					if(StringUtils.isNotBlank(impact) && !"null".equals(impact)){
						assessSample.setImpact(Double.valueOf(impact));
					}
					mergeAssessSample(assessSample);
				}else{
	...
	public void batchSetSampleIsQ(String jsonString, String isQualified){
		...
		for(int i=0;i<jsonArray.size();i++){
			...
			String assessSampleId = jsonObject.getString("assessSampleId");//评价结果ID
			AssessSample assessSample = o_assessSampleDAO.get(assessSampleId);
			if(null != assessSample && StringUtils.isNotBlank(isQualified)){
				assessSample.setIsQualified(isQualified);
	...
	*/
	...
	public void mergeAssessResultBatch(String assessorId, String jsonString,String assessPlanId,String processId,String orgId){
		...
		for(int i=0;i<jsonArray.size();i++){
			...
			String impact=jsonObject.getString("impact");//补充说明
			...
			Map<String, Object> assessSampleCountMap = this.findAssessSampleCount(assessResultList);
			for (AssessResult assessResult : assessResultList) {
				if(StringUtils.isNotBlank(assessResultId) && assessResultId.equals(assessResult.getId())){
					...
					*/
					...
					Map<String,Object> object = (Map<String, Object>) assessSampleCountMap.get(assessResult.getId());
					if(null != object){
						long y = (Long) object.get("Y");
						long n = (Long) object.get("N");
						if(0 != (y+n)){
							probability = (double) n/(y+n);
						}
					}
					assessResult.setProbability(probability);
					//影响程度
					if(StringUtils.isNotBlank(impact) && !"null".equals(impact)){
						assessResult.setImpact(Double.valueOf(impact));
					}else{
						assessResult.setImpact(null);
					}
					mergeAssessResult(assessResult);
	...
	public boolean isNotTest(AssessResult assessResult, List<AssessRelaDefect> assessRelaDefectList){
		...
		if(null == assessResult.getHasDefectAdjust()){
			...
			if(!assessResult.getHasDefectAdjust()){
				...
				if(null != assessResult.getHasDefect() && assessResult.getHasDefect()!=assessResult.getHasDefectAdjust() && StringUtils.isNotBlank(assessResult.getAdjustDesc())){
					//自动计算与人为调整不一致,并且补充说明不为空
					isDefect = true;
				}
			}else{
				//人为调整为是
	...
	*/
	public List<AssessSampleRelaFile> findAssessResultListByAssessSampleIds(List<String> assessSampleIds){
		Criteria criteria = o_assessSampleRelaFileDAO.createCriteria();
		criteria.createAlias("assessSample", "assessSample");
		if(!assessSampleIds.isEmpty()){
			criteria.add(Restrictions.in("assessSample.id", assessSampleIds));
		}
		return criteria.list();
	}
	 ...
	 * @author 张 雷
	 * @param assessResultId 评价结果ID
	 * @param isQualified 有效/无效/不适用的常量值
	 * @return
	 * @since  fhd　Ver 1.1
	*/
	...
	*/
	public Map<String,Object> findAssessSampleCount(List<AssessResult> assessResultList){
		...
		for(AssessResult assessResult:assessResultList){
			...
			assessResultMap.put(assessResultId, sampleMap);
		}
	...
	*/
	 ...
	 * @since  fhd　Ver 1.1
	*/
	public Integer findQualifiedSampleTestSampleNum(String sampletestIds) {
		String hql="select count(id) from AssessSample assessSample where assessSample.assessResult.id in (:assessResultId)  and assessSample.isQualified=:isQualified";
		Query query = o_assessSampleDAO.createQuery(hql);
		query.setParameterList("assessResultId", sampletestIds.split(","));
		query.setParameter("isQualified", Contents.IS_OK_Y);
