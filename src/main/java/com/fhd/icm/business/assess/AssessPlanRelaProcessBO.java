...
public class AssessPlanRelaProcessBO {
	...
	public void saveAssessPlanRelaProcess(String processIds,String assessPlanId,String assessPlanDepartIds,String selectType){
		...
		//根据processIdSet查询所有对应的流程
		List<Process> processList = o_processBO.findProcessByIds(StringUtils.join(processIdSet, ","));
		//根据processIdSet查询所有对应的流程责任部门和相关部门
		List<ProcessRelaOrg> processRelaOrgList = o_processBO.findProcessRelaOrgByProcessIds(StringUtils.join(processIdSet, ","));
		//根据processIdSet查询所有对应的流程风险状态
		Map<String, Double> coverageRateMap = o_assessPlanBO.findFrequenceNumMapByProcessList(StringUtils.join(processIdSet, ","));
		...
		for(String pId : processIdSet){
			...
			if(!flag){
				...
				//抽取样本比例
				assessPlanRelaProcess.setCoverageRate(coverageRateMap.get(pId));
				...
				if(null != assessPlan.getType() && Contents.ASSESS_MEASURE_ETYPE_SELF.equals(assessPlan.getType().getId())){//自评
					...
					for(ProcessRelaOrg processRelaOrg : processRelaOrgList){
						if(pId.equals(processRelaOrg.getProcess().getId()) && null!=processRelaOrg.getEmp() && !map.containsKey(processRelaOrg.getEmp().getId())){
	...
	*/
	...
	public void mergeAssessPlanRelaProcessForSampleTestBatch(String assessPlanProcessIds, Boolean isSampleTest, String jsonString){
		...
		List<AssessPlanRelaProcess> assessPlanRelaProcessList = o_assessPlanRelaProcessDAO.get(assessPlanProcessIdSet);
		
		Set<String> processIdSet = new HashSet<String>();
		for (AssessPlanRelaProcess assessPlanRelaProcess : assessPlanRelaProcessList) {
			if(!processIdSet.contains(assessPlanRelaProcess.getProcess().getId())){
				processIdSet.add(assessPlanRelaProcess.getProcess().getId());
			}
		}
		//根据processIdSet查询所有对应的流程风险状态
		Map<String, Double> coverageRateMap = o_assessPlanBO.findFrequenceNumMapByProcessList(StringUtils.join(processIdSet, ","));
		...
		for (AssessPlanRelaProcess assessPlanRelaProcess : assessPlanRelaProcessList) {
			if(null != isSampleTest){
				assessPlanRelaProcess.setCoverageRate(coverageRateMap.get(assessPlanRelaProcess.getProcess().getId()));
				assessPlanRelaProcess.setIsSampleTest(isSampleTest);
