...
public class StatisticFunctionCalculateBO {
	...
	private static final Logger LOGGER = Logger.getLogger(StatisticFunctionCalculateBO.class);
	...
	public static double vol(double[] source){
		...
		System.arraycopy(source, 0, params, 0, source.length-1);
		int t = Double.valueOf(source[source.length-1]).intValue();
		Variance variance=new Variance();
		return Math.sqrt(t)*Math.sqrt(variance.evaluate(params));
	}
	/**
	 * 同比与环比-变动率.
	 * @author 吴德福
	...
	public static void main(String[] args) {
		double[] source = new double[]{1.2,2.4,3.4,5.2,5.45};
		LOGGER.info(vol(source));
	}
}
