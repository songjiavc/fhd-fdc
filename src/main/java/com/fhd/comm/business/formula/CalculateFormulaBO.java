...
public class CalculateFormulaBO {
    ...
    private static Log logger = LogFactory.getLog(CalculateFormulaBO.class);
     ...
     * @return String
     */
    public String calculate(String targetId, String targetName, String type, String formula, String timePeriodId, String companyId, Map<String, Object> valueMapObj)
    {
        ...
        if (Contents.OBJECT_KPI.equals(type))
        {
            ...
            if (null != nameList && nameList.size() > 0)
            {
                 ...
                 * @param nameList
                 */
                List<KpiGatherResult> kpiGatherResultList = (List<KpiGatherResult>) valueMapObj.get("allKpiResultList");
        ...
        else if (Contents.OBJECT_CATEGORY.equals(type))
        {
            ...
            {
                return calculateSubFun(targetId, targetName, type, formula, timePeriodId, valueMapObj);
            }
            if (formula.indexOf("SumWeightAverage") != -1)
            {
                return calculateSumWeightAverage(targetId, targetName, type, formula, timePeriodId, valueMapObj);
            }
            ...
            {//记分卡公式支持通过其它记分卡配置
                ...
                if (null != nameList && nameList.size() > 0)
                {
                    ...
                    if (codeList.size() > 0)
                    {
                        assessResultList = findAssessResultByCodeList(codeList, (List<RelaAssessResultDTO>) valueMapObj.get("allCategoryGatherResult"));
                    }
        ...
        else if (Contents.OBJECT_STRATEGY.equals(type))
        {
            ...
            {//战略目标公式支持通过其它战略目标配置
                ...
                if (null != nameList && nameList.size() > 0)
                {
                    ...
                    if (codeList.size() > 0)
                    {
                        assessResultList = findAssessResultByCodeList(codeList, (List<RelaAssessResultDTO>) valueMapObj.get("allStrategyGatherResult"));
                    }
    ...
    public String calculateIsKpiStatusFun(String targetName, String subFormula, String timePeriod)
    {
        ...
        if (null != contentArray && contentArray.length == 2)
        {
            ...
            if ("0frequecy_relatime".equals(frequence))
            {//实时指标查询最晚的采集结果
     ...
     * @return
     */
    public String calculateSumWeightAverage(String targetId, String targetName, String type, String subFormula, String timePeriod, Map<String, Object> valueMapObj)
    {
        String ret = "";
        Double sum = 0.0d;
        ...
        {//记分卡
         //objectList =  o_kpiGatherResultBO.findKpiGatherResultsListByCategoryId(targetId, contentArray[1], timePeriod);
            objectList = (List<Object[]>) valueMapObj.get("categoryRelaKpiResultList");

        }
        else if (Contents.OBJECT_STRATEGY.equals(type))
        {
            //objectList = o_kpiGatherResultBO.findKpiGatherResultsListByStrategyId(targetId, contentArray[1], timePeriod);
            objectList = (List<Object[]>) valueMapObj.get("strategyMapRelaKpiResultList");
        }
        if (null != objectList && objectList.size() > 0)
        {
            for (Object[] object : objectList)
            {
                if ("评估值".equals(contentArray[1]))
                {
                    if (object[1] != null && object[11] != null)
                    {
                        Double value = Double.valueOf(String.valueOf(object[1]));
                        Double weight = Double.valueOf(String.valueOf(object[11]));
                        sumWeight += weight;
                        sum += value * weight;
                    }
                    else
                ...
                else if ("实际值".equals(contentArray[1]))
                {
                    if (object[2] != null && object[11] != null)
                    {
                        Double value = Double.valueOf(String.valueOf(object[2]));
                        Double weight = Double.valueOf(String.valueOf(object[11]));
                        sumWeight += weight;
                        sum += value * weight;
                    }
     ...
     */
    public String calculateSubFun(String targetId, String targetName, String type, String subFormula, String timePeriod, Map<String, Object> valueMapObj)
    {
        DecimalFormat df = new DecimalFormat("0.00");
        String funContent = subFormula.substring(subFormula.indexOf("(") + 1, subFormula.indexOf(")"));
        String[] contentArray = funContent.split(",");
        ...
        if (null != contentArray && contentArray.length > 0)
        {
            ...
            if (Contents.OBJECT_CATEGORY.equals(type))
            {
                ...
                {
                    //通过下级记分卡计算
                    objectList = (List<Object[]>) valueMapObj.get("categoryRelaSubCategoryResultList");
            ...
            else if (Contents.OBJECT_STRATEGY.equals(type))
            {
                ...
                {
                    //通过下级战略目标计算
                    objectList = (List<Object[]>) valueMapObj.get("strategyMapRelaSubCategoryResultList");
    ...
    /**根据公式中配置的记分卡code集合从所有记分卡采集数据中找到相应的采集数据
     * @param codeList 公式中配置的记分卡code集合
     * @return
     */
    private List<RelaAssessResultDTO> findAssessResultByCodeList(List<String> codeList, List<RelaAssessResultDTO> assessResultList)
    {
        List<RelaAssessResultDTO> selectAssessResultList = new ArrayList<RelaAssessResultDTO>();
        for (String code : codeList)
        {
            for (RelaAssessResultDTO result : assessResultList)
            {
                if (result.getObjectCode().equals(code))
                {
                    selectAssessResultList.add(result);
                }
            }
        }
        return selectAssessResultList;
    }
}
