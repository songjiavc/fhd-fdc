            ...
            if (Contents.OBJECT_KPI.equals(objectType)) {
                ...
                if (null != currentKpiGatherResult) {
                    ...
                    if (Contents.TARGET_VALUE_FORMULA.equals(objectColumn)) {
                        computeFlag = true;
                        String ret = o_formulaCalculateBO.calculate(kpi.getId(),kpi.getName(), Contents.OBJECT_KPI, kpi.getTargetFormula(), timePeriodId,kpi.getCompany().getId());
                        if (o_formulaCalculateBO.isDoubleOrInteger(ret)) {
                            ...
                            if (computeFlag) {
                                ...
                                String kpiNewValue = o_kpiBO.findKpiNewValue(kpi.getId());
                                if (null != kpiNewValue) {
                    ...
                    if (Contents.RESULT_VALUE_FORMULA.equals(objectColumn)) {
                        computeFlag = true;
                        String ret = o_formulaCalculateBO.calculate(kpi.getId(),kpi.getName(), Contents.OBJECT_KPI, kpi.getResultFormula(), timePeriodId,kpi.getCompany().getId());
                        if (o_formulaCalculateBO.isDoubleOrInteger(ret)) {
                            //返回结果是double，计算正确
                    ...
                    if (Contents.ASSESSMENT_VALUE_FORMULA.equals(objectColumn)) {
                        computeFlag = true;
                        String ret = o_formulaCalculateBO.calculate(kpi.getId(),kpi.getName(), Contents.OBJECT_KPI, kpi.getAssessmentFormula(), timePeriodId,kpi.getCompany().getId());
                        if (o_formulaCalculateBO.isDoubleOrInteger(ret)) {
                            //返回结果是double，计算正确
                ...
                if(StringUtils.isNotBlank(formular)){
                    ...
                    if (o_formulaCalculateBO.isDoubleOrInteger(result)) {
                        ...
                        if(null!=alarmPlan){
                            Set<AlarmRegion> alarmRegionSet = alarmPlan.getAlarmRegions();
