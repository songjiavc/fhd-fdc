...
import com.fhd.entity.comm.report.ReportRelaConstructPlan;
...
//			report.setReportCode(reportCode);
		...
		o_constrcutPlanReportInfomationBO.saveInternalControlManual(report, request);
...
//	@RequestMapping("/comm/report/downloadReport.f")
	public void downloadReport(String id, OutputStream os, HttpServletResponse response) throws Exception {
		...
		if(null != reportInfomation.getFile()){
			...
			response.setHeader("Content-Disposition", "attachment;filename="+ URLEncoder.encode(reportInfomation.getReportName()+".doc", "UTF-8"));
		
			IOUtils.write(reportInfomation.getFile().getContents(), os);
		}
	}
}