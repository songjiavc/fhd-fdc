...
public class ReportInfomationControl {
	...
	public Map<String,Object> saveCompanyYearReport(String id, String name, String code, String assessplanId, String reportData, String reportType, HttpServletRequest request) throws Exception{
		...
		o_reportInfomationBO.saveReportByFtl(report, assessplanId, request);
    ...
    public Map<String,Object> findReportInfomationList(int limit, int start, String sort, String query, String reportType){
		...
		List<ReportInfomation> reportInfomationList = page.getResult();
		if(!reportInfomationList.isEmpty()){
			Map<String, Object> row = null;
			for(ReportInfomation ri : reportInfomationList){
				row = new HashMap<String, Object>();
				//id
				row.put("id", ri.getId());
				//版本号
				row.put("code", ri.getReportCode());
				//标题
				row.put("name", ri.getReportName());
				//公司名称
				if(null != ri.getCompany()){
					row.put("companyName", ri.getCompany().getOrgname());
				}else{
					row.put("companyName", UserContext.getUser().getCompanyName());
				}
				//状态
				row.put("status", ri.getStatus());
				//创建人
				if(null != ri.getCreateBy()){
					row.put("createEmp", ri.getCreateBy().getEmpname());
				}else{
					row.put("createEmp", "");
				}
				//执行状态
				row.put("dealStatus", ri.getExecuteStatus());
				//创建时间
				if(null != ri.getCreateTime()){
					row.put("createDate", DateUtils.formatDate(ri.getCreateTime(), "yyyy-MM-dd HH:mm:ss"));
				}else{
					row.put("createDate", "");
				}
				//修改人
				if(null != ri.getLastModifyBy()){
					row.put("modifyEmp", ri.getLastModifyBy().getEmpname());
				}else{
					row.put("modifyEmp", "");
				}
				//修改时间
				if(null != ri.getLastModifyTime()){
					row.put("modifyDate", DateUtils.formatDate(ri.getLastModifyTime(), "yyyy-MM-dd HH:mm:ss"));
				}else{
					row.put("modifyDate", "");
				}
				//关联文件
				if(null != ri.getFile()){
					row.put("fileId", ri.getFile().getId());
				}else{
					row.put("fileId", "");
				}
				datas.add(row);
	...
	public void downloadReport(String id, OutputStream os, HttpServletResponse response) throws Exception {
		FileUploadEntity fileUpload = o_fileUploadBO.findById(id);
		response.setContentType("doc");
		response.setHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode(fileUpload.getNewFileName()+".doc", "UTF-8"));
	...
	/**
	 * 查询评价报告下缺陷整改情况表.
	 * 增加query参数过滤 郑军祥6-6修改
	 * @param assessplanId
	 * @return Map<String,Object>
	 */
	@ResponseBody
    @RequestMapping("/comm/report/findReportDefectImproveInfoList.f")
    public Map<String,Object> findReportDefectImproveInfoList(String assessplanId,String query){
		Map<String, Object> map = new HashMap<String, Object>();
		List<Map<String, Object>> datas = new ArrayList<Map<String, Object>>();
		
		List<Object[]> list = o_improvePlanRelaDefectBO.findDefectImproveInfoByAssessplanId(assessplanId,query);
	 ...
	 */
	@ResponseBody
    @RequestMapping("/comm/report/findRiskManageReport.f")
    public Map<String,Object> findRiskManageReport(int limit, int start, String sort, String query){
		Map<String, Object> resultMap = new HashMap<String, Object>();
//		List<Map<String, Object>> datas = new ArrayList<Map<String, Object>>();
		List<Map<String,Object>> list = o_reportInfomationBO.findReportByReportType(Contents.REPORT_RISK_MANAGE);
//		for(ReportInfomation report : list){
//			Map<String,Object> map = new HashMap<String,Object>();
//			map.put("id", report.getId());
//			map.put("code", report.getReportCode());
//			map.put("name", report.getReportName());
//			map.put("company", report.getCompany().getOrgname());
//			map.put("creator", report.getCreateBy().getEmpname());
//			map.put("createTime", report.getCreateTime().toLocaleString());
//			datas.add(map);
//		}
	 ...
	 */
	@ResponseBody
    @RequestMapping("/comm/report/findRiskAssessReport.f")
    public Map<String,Object> findRiskAssessReport(int limit, int start, String sort, String query){
		Map<String, Object> resultMap = new HashMap<String, Object>();
//		List<Map<String, Object>> datas = new ArrayList<Map<String, Object>>();
		List<Map<String,Object>> list = o_reportInfomationBO.findReportByReportType(Contents.REPORT_RISK_ASSESS);
//		for(ReportInfomation report : list){
//			Map<String,Object> map = new HashMap<String,Object>();
//			map.put("id", report.getId());
//			map.put("code", report.getReportCode());
...
//		}
	...
	/**
	 * 查询已经完成的风险评估计划，供风险报告生成时使用
	 * @author zhengjunxiang 2013-9-27
	 * @return Map<String,Object>
	 */
	@ResponseBody
    @RequestMapping("/comm/report/findFineshedAssessPlan.f")
    public Map<String,Object> findFineshedAssessPlan(){
		Map<String, Object> resultMap = new HashMap<String, Object>();
		String finishFlag = "F";
		List<RiskAssessPlan> list = o_riskAssessPlanBO.findByDealStatusOrderTime(finishFlag);
		
		List<Map<String,Object>> datas = new ArrayList<Map<String,Object>>();
		for(RiskAssessPlan p : list){
		...
		resultMap.put("totalCount", datas.size());
