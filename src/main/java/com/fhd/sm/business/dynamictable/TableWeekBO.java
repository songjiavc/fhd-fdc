...
import com.fhd.sm.web.controller.util.TableWeekUtil;
...
public class TableWeekBO {
     ...
     * @since  fhd　Ver 1.1
    */
    public String getWeekHtml(String contextPath, String year, boolean isEdit, String kpiId, String kpiName, String timeId,String containerId){
...
//		add by haojing  备注信息列
	    kpiName = "采集信息";
	    String titleHtml = TableWeekUtil.getStartTableHtml(isEdit, kpiName,containerId);
...
//		add by haojing  备注信息列
		...
		for (TimePeriod monthEn : monthList) {
			...
			//创建月
			if(monthNumeral == 12){
				monthHtml = TableWeekUtil.getMonthHtml(weekInt + 1 + 1, yearNumeral, monthNumeral,containerId);
			}else{
				monthHtml = TableWeekUtil.getMonthHtml(weekInt + 1, yearNumeral, monthNumeral,containerId);
			}
...
//				add by haojing  备注信息列
				if(kpiGatherResultWeek != null){
					kgrId = kpiGatherResultWeek.getId();
					objList = o_kpiMemoBO.findMemoByKgrId(kgrId);
			 		memoForm = new KpiMemoForm(objList);
			 		icon = memoForm.getImportant();
			 		if(icon!=null&&icon.equals("0alarm_startus_h")){
