...
import com.fhd.fdc.commons.compass.AdvancedSearchCommand.CompassSort;
...
public class CompassSearchService implements InitializingBean {
	...
	protected CompassSearchResults performSearch(CompassSearchCommand searchCommand, CompassSession session) {
		...
		if (pageSize == null) {
			...
			if (from > hits.getLength()) {
				...
				// 中间的页码，直接取出相应的条目
				doProcessBeforeDetach(searchCommand, session, hits, from,from + iPageSize);
		...
		CompassSearchResults searchResults = new CompassSearchResults(detachedHits.getHits(), time,hits.getLength());
	...
	protected CompassQuery buildQuery(CompassSearchCommand searchCommand, CompassSession session) {
		...
		if (AdvancedSearchCommand.class.isAssignableFrom(searchCommand.getClass())) {
			...
			if(advancedSearchCommand.getTypes().length > 0) {
				query.setTypes(advancedSearchCommand.getTypes());
			}
