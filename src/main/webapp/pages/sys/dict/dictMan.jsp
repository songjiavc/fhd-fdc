...
<script type="text/javascript">
	...
	var gridColums = [
...

		{
			...
			renderer : function(value, metaData, record, colIndex, store,
				...
				if (issys == "1") {
					return "<span style='color:#C0C0C0' class=''>" + value
		...
		},
		{
			...
			renderer : function(value, metaData, record, colIndex, store,
				...
				if (issys == "1") {
					return "<div style='color:#C0C0C0' class=''>" + num
							+ "</div>";
				} else {
		...
		},
		{
			...
			renderer : function(value, metaData, record, colIndex, store,
				...
				if (issys == "1") {
					return "<div style='color:#C0C0C0' class=''>" + value
							+ "</div>";
				} else {
					return value;
				}
	...
	var dictTreeComboxStore = Ext.create('Ext.data.TreeStore', {//dictTreeCombox的store
		...
		proxy : {
			type : 'ajax',
			url : 'sys/dic/findDictEntryBySome.f'/*加载dictTree的url*/
		},
		autoload : false
	});
	/***attribute end***/
	...
	function edit(button) {//新增方法
		...
		if (button.id == 'dictadd${param._dc}') {
			dictTreeComboxStore.proxy.url = dictGrid.store.proxy.url
					+ '?typeId=' + curTypeId;
		} else {
			dictTreeComboxStore.proxy.url = dictGrid.store.proxy.url
					+ '?typeId=' + curTypeId + '&entryId=' + curEntryId;
		}
		...
		formwindow = new Ext.Window({
			...
			autoLoad : {
				url : 'pages/sys/dict/dictedit.jsp',
				nocache : true,
				scripts : true
			}
		});
	...
	function setstatus() {//设置你按钮可用状态
		...
		dictGrid.down('#dictedit${param._dc}').setDisabled(
				length == 0 || length > 1 || curEntryIsSystem == 1);
		dictGrid.down('#dictdel${param._dc}').setDisabled(
				length == 0 || curEntryIsSystem == 1);
	...
	Ext.onReady(function() {
		...
		dictTree = Ext.create('FHD.ux.TreePanel', {//实例化一个grid列表
			...
			height : FHD.getCenterPanel().getHeight() - 5,
			listeners : {
				'itemclick' : function(view, re) {
					curTypeId = re.data.id;
					dictGrid.getSelectionModel().deselectAll();
					dictGrid.store.proxy.extraParams = [ {
						'typeId' : re.data.id
					} ];
					...
					dictGrid.down('searchfield').triggerCell.item(0)
							.setDisplayed(false);
					if (loadable) {
						dictGrid.store.load({
							callback : doLoad
		...
		dictGrid = Ext.create('FHD.ux.TreeGridPanel', {//实例化一个grid列表
			...
			checked : false,
			autoScroll : true,
			width : FHD.getCenterPanel().getWidth() - 225,
			height : FHD.getCenterPanel().getHeight() - 3,//高度为：获取center-panel的高度
		...
		mainPanel = Ext.create('Ext.container.Container', {
			...
			border : false,
			//2017年3月30日10:02:38 吉志强 数据字典报错修改
			//height : FHD.getCenterPanel().getHeight()-FHD.getCenterPanel().getTabBar().getHeight()-3-1,
			height : FHD.getCenterPanel().getHeight()-3-1,
			layout : {
				type : 'border'
			},
