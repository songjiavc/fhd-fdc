Ext.define('FHD.demo.editgrid.EditGridList', {
	...
	initComponent : function() {
		...
		function onchange() {// 设置你按钮可用状态
			...
			grid.down('#save')
					.setDisabled(grid.store.getModifiedRecords().length === 0);
		}
		...
		Ext.apply(Ext.form.field.VTypes, {
					...
					uniqueTitle : function(val, field) {
						var id = field.up("grid").getSelectionModel()
					...
					treeNode : function(val, field) {
						var flag = false;
						var selection = field.up("grid").getSelectionModel()
								.getSelection()[0];
