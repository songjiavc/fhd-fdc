Ext.define('FHD.demo.grid.GridList', {
	...
	initComponent : function() {
		...
		var gridColums = [{
			...
			renderer : function(value, metaData, record, colIndex, store, view) {
				/*
				 * data-qtip:设置提示正文内容。 data-qtitle:设置提示的标题。 data-qwidth:设置提示的宽度。
				 * data-qalign:表示用提示的一个基准点，对应到原件的哪个基准点。例如：tl-br表示用提示的左上角，对应到原件的右下角。
				 */
				metaData.tdAttr = 'data-qtip="' + value + '" data-qwidth="'
						+ 10 + '"';
				return "<a href=\"javascript:void(0);\" onclick=\"view('"
						+ record.get('id') + "')\">" + value + "</a>";
			}
		}, {
			header : FHD.locale.get('fhd.pages.test.field.title'),
			dataIndex : 'title',
			sortable : true,
			width : 60,
			flex : 1
		}, {
			header : FHD.locale.get('fhd.pages.test.field.myLocale'),
			dataIndex : 'myLocale',
			sortable : true,
			width : 60,
			renderer : function(value) {
				var index = me.fieldstore.find('id', value);
				var record = me.fieldstore.getAt(index);
				if (record != null) {
			...
			header : FHD.locale.get('fhd.pages.test.field.parentName'),
			dataIndex : 'parentId',
			sortable : true,
			width : 160,
			renderer : function(value) {
				var record = me.treeComboxStore.getNodeById(value);
				if (record != null) {
			...
			header : FHD.locale.get('fhd.pages.test.field.level'),
			dataIndex : 'level',
			sortable : true,
			width : 60
		}];
		...
		function edit(button) {// 新增方法
			...
			var gridEditPanel = Ext.create('FHD.demo.grid.GridEdit');
						...
						FHD.ajax({// ajax调用
							...
							callback : function(data) {
								if (data) {// 删除成功！
									...
									me.treeComboxStore.load();// 重新加载treeCombox
									grid.store.load();
								}
							}
						});
		...
		function setstatus() {// 设置你按钮可用状态
			grid.down('#del').setDisabled(grid.getSelectionModel()
		...
		indexMainPanel.tree.on('collapse', function(p) {
					grid.setWidth(indexMainPanel.getWidth() - 26 - 5);
				});
		indexMainPanel.tree.on('expand', function(p) {
					grid.setWidth(indexMainPanel.getWidth() - p.getWidth() - 5);
				});
		...
		indexMainPanel.tree.on('resize', function(p) {
					if (p.collapsed) {
						...
						grid.setWidth(indexMainPanel.getWidth() - p.getWidth()
								- 5);
					}
				});
		Ext.applyIf(me, {
