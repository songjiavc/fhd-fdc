<%@page import="org.hibernate.stat.ConcurrentSecondLevelCacheStatisticsImpl"%>
<%@ page language="java" contentType="text/html;charset=UTF-8"%>
<%@ include file="head.jsp" %><html>
<head>
...
<table cellpadding="5" cellspacing="1"  bgcolor="#26AEF0" style="font-size: 12px" width="80%">
  ...
  <tr bgcolor="white">
    <td>缓存中的对象个数:</td>
    <td><fmt:formatNumber value="${statistics.secondLevelCachePutCount}"/></td>
  </tr>
<%
  ...
  for(int i=0; i<statistics.getSecondLevelCacheRegionNames().length; i++) {
    String queryName = statistics.getSecondLevelCacheRegionNames()[i];
...
<%
  for(int i=0; i<querys.length; i++) {
	ConcurrentSecondLevelCacheStatisticsImpl secondLevelCacheStatistics = (ConcurrentSecondLevelCacheStatisticsImpl)querys[i];
...
%>
  <tr bgcolor="#F5FBFE">
    <td colspan="2">缓存名称: <%=secondLevelCacheStatistics.getCategoryName() %></td>
  </tr>
  <tr bgcolor="white">
    <td>命中率:</td>
    <td>
