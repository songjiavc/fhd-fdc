		...
		items : [{
					...
					items : [{
								...
								items : [{
									...
									iconCls : "reset",
									handler : function() {
										var d = Ext.getCmp("resumefileIds");
										var c = Ext.getCmp("resumeFilePanel");
										c.body.update("");
										d.setValue("");
									}
								}, {
	...
	if (this.resumeId != null && this.resumeId != "undefined") {
		a.getForm().load({
			...
			success : function(d, f) {
				var h = Ext.util.JSON.decode(f.response.responseText).data[0];
				var c = h.photo;
				var k = h.resumeFiles;
				...
				if (e != null) {
					Ext.getCmp("ResumeForm.startWorkDate")
							.setValue(new Date(getDateFromFormat(e,
									"yyyy-MM-dd HH:mm:ss")));
				}
				if (m != null) {
					Ext.getCmp("ResumeForm.birthday")
							.setValue(new Date(getDateFromFormat(m,
									"yyyy-MM-dd HH:mm:ss")));
				}
