 ...
 * 整合计划制定入口
 */
Ext.define('FHD.view.risk.planconform.PlanConformFormMain',{
 	extend: 'Ext.form.Panel',
 	alias : 'widget.planConformFormMain',
 	border:false,
 	layout:{
		align: 'stretch',
		type: 'border'
    ...
    initComponent: function () {
    	var me = this;
    	//计划制定第一步
    	me.planConformEditOne = Ext.create('FHD.view.risk.planconform.PlanConformEditOne',{
		 	navigatorTitle:'计划制定',
		 	border: false,
		 	...
		 	last:function(){//保存计划
    		 	...
    		 	if(form.isValid()){
    		 		FHD.submit({
						form:form,
						url: __ctxPath + '/access/planconform/saveplanbyplantype.f',
						callback:function(data){
    	...
    	if (null!=me.businessId) {
    		  	me.planConformEditOne.loadData(me.businessId);//加载表单数据
    	}
		me.planConformEditNext = Ext.create('FHD.view.risk.planconform.PlanConformEditNext',{
			 navigatorTitle:'范围选择',
			 /*
			  * 传递分库标识
			  * change by 郭鹏
			 ...
			 back:function(){//上一步方法，传参
			 	...
			 	me.finishTop.setDisabled(false);
			 }
		});
		...
		me.basicPanel = Ext.create('FHD.ux.layout.StepNavigator',{
			...
			btns: [me.btnSubmit],
		 	...
		 	undo : function(){
		 		...
		 		var cardPanel = me.up('planConformCard');
		 		setTimeout(function(){
		 			if(me.executionId)
		 				{
