 ...
 * 风险-目标 列表页面
 * 
 * @time 2017年3月22日15:18:08
 */

Ext.define('FHD.view.risk.riskstorage.RiskTargetEventGridNew', {
    extend: 'FHD.ux.GridPanel',
    alias: 'widget.risktargeteventgridnew',
    requires: [
    ...
    addFun: function () {
        var me = this;
        me.up('risktargetcardnew').riskRelateForm.resetData();
        //获取选中的树节点
        var treeObj = me.up('risktargetmainpanelnew').targetTree;
        var node = treeObj.getCurrentTreeNode();
        if(node != null && node !=''){
        	 //获取不包含父节点的id 即dbid
            var selectedId = node.data.dbid;
            //赋值 表单中的指标信息（select框）
            me.up('risktargetcardnew').riskRelateForm.influKpiName.initGridStore(selectedId);
        }
       
        
        me.up('risktargetcardnew').showRiskRelateForm();
    },
    ...
    editFun: function () {
        ...
        //切换到风险事件添加
        me.up('risktargetcardnew').riskRelateForm.reloadData(id);
        me.up('risktargetcardnew').showRiskRelateForm();
    },
