Ext.define('FHD.view.risk.strategy.StrategyWarningSet', {
    extend: 'FHD.ux.EditorGridPanel',
    alias: 'widget.strategywarningset',
    ...
    _validateContent: function (jsonArray) {
        for (var i = 0; i < jsonArray.length; i++) {
            ...
            if((jsonArray[i].alarm==null ||jsonArray[i].alarm == "" ) &&(jsonArray[i].warning==null||jsonArray[i].warning == "")){
            	Ext.ux.Toast.msg(FHD.locale.get('fhd.common.prompt'),"需要设置告警或预警方案!");
            	return false;
            }
        }
        return true;
    ...
    last: function (cardPanel, finishflag) {
        ...
        if (me._validateContent(jsonArray)) {
            ...
            FHD.ajax({
                url: __ctxPath + '/kpi/kpistrategymap/mergestrategyrelaalarm.f',
                params: {
                ...
                callback: function (data) {
                    if (data && data.success) {
                        ...
                        if (!finishflag) { //如果点击的不是完成按钮,需要移动到下一个面板
                            ...
                            strategyobjectivecardpanel.navBtnState(cardPanel); //设置导航按钮状态,如果是首个面板则上一步按钮为置灰状态,如果是最后一个面板则下一步按钮为置灰状态
                        } else {
    ...
    initComponent: function () {
        ...
        Ext.apply(me, {
            pagable: false,
            url: __ctxPath + '/kpi/kpistrategymap/findsmrelaalarmbysome.f',
            extraParams: {
                currentSmId: me.currentSmId,
        ...
        me.on('selectionchange', function () {
            //删除按钮
            if (me.down('#sm_delBtn')) {
                me.down('#sm_delBtn').setDisabled(me.getSelectionModel().getSelection().length === 0);
            }
        });
