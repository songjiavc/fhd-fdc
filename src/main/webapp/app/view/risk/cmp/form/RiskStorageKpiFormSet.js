Ext.define('FHD.view.risk.cmp.form.RiskStorageKpiFormSet', {
    ...
    url: __ctxPath + '/cmp/risk/findkpirelarisk.f',//'/kpi/kpistrategymap/findsmrelakpibysome.f',
    extraParams: {
        riskId: ''
    },
    cols: [{
            ...
            callback: function (data) {
                if (data && data.kpiRelOrg) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        list[i].set("unit", vobj[list[i].get("id")].unit);
                        list[i].set("frequence", vobj[list[i].get("id")].frequence);
                        me.store.add(list[i]);
                    }
                }
            }
    ...
    reloadData: function () {
        var me = this;
        me.store.proxy.extraParams.riskId = me.paramObj.riskId;
        me.store.load();
    }
