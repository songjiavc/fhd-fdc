 ...
 * @author 张健
 */
Ext.define('FHD.view.risk.cmp.container.analyse.AnalyseProcessContainer', {
    extend: 'FHD.ux.CardPanel',
    alias: 'widget.analyseprocesscontainer',
    ...
    chageNavigationBar: function (nodeId, name) {
        var me = this;
        if (me.navHeight != 0) {
            me.navigationBar.renderHtml(me.tabContainer.id + 'DIV', nodeId, name, 'process', me.navFunId);
        }
    },
    //风险列表相关导航操作
    chageRiskNavigationBar: function (navId, nodeId, name) {
        var me = this;
        if (me.navHeight != 0) {
            me.navigationBar.renderHtml(navId, me.currentId, name, 'process', me.navFunId);
        }
    },
    ...
    initComponent: function () {
        ...
        var treeArr = [];
        //图表分析
        if($ifAnyGranted('ROLE_ALL_ASSESS_ANALYSIS_RISKQUERY_PROCESS_CHART')){
        ...
        //风险事件列表页
        if($ifAnyGranted('ROLE_ALL_ASSESS_ANALYSIS_RISKQUERY_PROCESS_LIST')){
            me.riskEventGridContainer = Ext.create('Ext.container.Container', {
                ...
                onClick: function () {
                    ...
                    //根据左侧选中节点，初始化数据
                    if (me.currentId != '') {
                        me.riskEventGrid.initParams('process');
        ...
        //历史记录页
        if($ifAnyGranted('ROLE_ALL_ASSESS_ANALYSIS_RISKQUERY_PROCESS_HISTORY')){
