 ...
 * 权限控制在风险库维护中
 */
Ext.define('FHD.view.risk.cmp.risk.RiskEventAddApproveGrid', {
    extend: 'FHD.ux.CardPanel',
    alias: 'widget.riskeventaddapprovegrid',
    requires: [
        'FHD.view.risk.assess.utils.GridCells'
    ],

    //按不同的状态查询风险事件. 待审批：waitingApprove；带归档：waitingArchive
    state: 'waitingApprove', 
    ...
    delFun: function () {
        var me = this;
        var delUrl = '/risk/flow/mergeRiskEventArchiveState';
        var selections = me.grid.getSelectionModel().getSelection();
        	...
        	Ext.MessageBox.show({
                    ...
                    fn: function (btn) {
                        if (btn == 'yes') { //确认删除
                            ...
                            FHD.ajax({ //ajax调用
                                url: __ctxPath + delUrl + "?state=deleted&ids=" + ids.join(','),
