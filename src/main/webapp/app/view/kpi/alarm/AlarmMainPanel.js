Ext.define('FHD.view.kpi.alarm.AlarmMainPanel', {
                    ...
                    FHD.ajax({ //ajax调用
                        ...
                        callback: function (data) {
                            if (data && data.success) { //删除成功！
                                ...
                                Ext.MessageBox.alert(FHD.locale.get('fhd.common.prompt'),"方案被引用，不能删除。");
                            }
                        }
                    });
                }
    ...
    /*编辑告警*/
    edit: function (button,flag) {
        var me = this;
        var selections = me.alarm_mgr_grid.getSelectionModel().getSelection();
        var length = selections.length;
        if (length >= 2) {
            Ext.MessageBox.alert(FHD.locale.get('fhd.common.prompt'), FHD.locale.get('fhd.alarmplan.prompt.editAlone'));
            return;
        }
    ...
    setstatus: function () { //设置你按钮可用状态
        ...
        if(me.alarm_mgr_grid.down("[name='del']")){
	        me.alarm_mgr_grid.down("[name='del']").setDisabled(length === 0);
        }
        if(me.alarm_mgr_grid.down("[name='edit']")){
	        me.alarm_mgr_grid.down("[name='edit']").setDisabled(length === 0);
        }
    },
            ...
            callback: function (data) {
                if (data && data.success) {
                	...
                	for(var k in data.regions){
                		if(k=='high'){
                			flag = true;
                			high = '<tr><td style="text-align: center;border: 2px #99bbe8 solid;" title="'+FHD.locale.get("fhd.alarmplan.form.hight")+'"><img src="' + __ctxPath + '/images/icons/symbol_high_sm.gif"></td><td style="text-align: center;border: 2px #99bbe8 solid;">&nbsp;'+data.regions[k]+'</td></tr>';
                		}else if(k=='mid'){
                			flag = true;
                			mid = '<tr><td style="text-align: center;border: 2px #99bbe8 solid;" title="'+FHD.locale.get("fhd.alarmplan.form.min")+'"><img src="' + __ctxPath + '/images/icons/symbol_mid_sm.gif"></td><td style="text-align: center;border: 2px #99bbe8 solid;">&nbsp;'+data.regions[k]+'</td></tr>';
                		}else if(k=='low'){
                			flag = true;
                			low = '<tr><td style="text-align: center;border: 2px #99bbe8 solid;" title="'+FHD.locale.get("fhd.alarmplan.form.low")+'"><img src="' + __ctxPath + '/images/icons/symbol_low_sm.gif"></td><td style="text-align: center;border: 2px #99bbe8 solid;">&nbsp;'+data.regions[k]+'</td></tr>' ;
                		}else if(k=='safe'){
                			flag = true;
