...
Ext.define('FHD.view.kpi.cmp.GridMergeCell', {
    alias: 'widget.gridCells',
    ...
    mergeCells : function(grid,cols){
    	if(document.getElementById(grid.getId()+"-body").firstChild.firstChild == null){
    		return;
    	}
            ...
            for(var i=0;i<trCount;i++){  //i=0表示表头等没用的行  
              ...
              Ext.each(arrayTd,function(Td){ //获取RowNumber列和check列  
                ...
                if(!divHtml){  
                    ...
                    if(cellText == divHtml){  
                        if(colIndex!=cols[0]){//cols[0]
                            var leftDisplay=arrayTd[1].style.display;//判断左边单元格值是否已display  
                            if(leftDisplay=='none') 
                            { 
                                addf();   }
                            else  
