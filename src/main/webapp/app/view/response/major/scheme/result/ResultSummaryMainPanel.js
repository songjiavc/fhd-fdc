 ...
 * 部门汇总主页面
 */
Ext.define('FHD.view.response.major.scheme.result.ResultSummaryMainPanel', {
    extend: 'Ext.form.Panel',
    alias: 'widget.resultsummarymainpanel',
    requires: [
	],
	//获得导航item
	getItems: function(){
		...
		me.flowtaskbarItem = Ext.create('FHD.ux.icm.common.FlowTaskBar',{
    		jsonArray:[
	    		...
	    		{index: 10, context:'10.方案审批(风管办公室)',status:'done'},
	    		{index: 11, context:'11.结果整理',status:'current'}
	    	],
	    	margin : '5 5 5 5'
		});
	},
		...
		Ext.MessageBox.show({
    		...
    		fn : function(btn) {
    			if (btn == 'yes') {//确认
    				...
    				//提交计划审批，执行流程
    				FHD.ajax({
    					url:__ctxPath + "/majorResponse/finishProcess",
    					params:{
    ...
    initComponent: function() {
        ...
        me.getItems();
	  	...
	  	var schemeType = "1";//汇总
	  	var empType = "2";//风险管理员
	  	me.DownPanel = Ext.create('FHD.view.response.major.scheme.result.ResultDownFormPanel',{
	  		flex: 1,
	  		businessId : me.businessId,
	  		executionId: me.executionId,
