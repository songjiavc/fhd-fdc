Ext.define('FHD.view.response.workplan.workplanmake.WorkPlanMakeGrid', {
    extend: 'FHD.ux.GridPanel',
    alias: 'widget.workPlanMakeGrid',
 	requires: [
 	           'FHD.view.IndexNav',
    ...
    edit : function(isAdd,isBtn){
    	...
    	var workPlanMakeEditMain = prt.workPlanMakeEditMain.p1;
    	var responsPerson = workPlanMakeEditMain.reponser;
    	var contactPerson = workPlanMakeEditMain.contactor; 
    	var btnSubmit = workPlanMakeEditMain.btnSubmit;//提交按钮置灰
    	if(btnSubmit){
    	...
    	if(isAdd){//添加按钮
    		...
    		workPlanMakeEditMain.getForm().reset();
    		workPlanMakeEditMain.workType.setValue('assess_work_type_project');//工作类型默认值‘应对方案制定’
    		var newBusinId;
    		...
    		var length = selection.length;
    	    	   if (length >= 2) {//判断是否多选
    					...
    					if(typeof(workPlanMakeEditMain) != 'undefined'){
    						responsPerson.clearValues();//清空组件缓存
    						contactPerson.clearValues();
    						workPlanMakeEditMain.getForm().reset();
    						me.businessId = selection[0].get('id');
    						workPlanMakeEditMain.loadData(me.businessId);
    					}
    					if(isBtn){
    ...
    previewWindow:function(me,pid){
    	var prt = me.up('workPlanMakeCard');
    	var workPlanMakeEditMain = prt.workPlanMakeEditMain;
    ...
    initComponent: function() {
        ...
        Ext.apply(me,{
        	region:'center',
        	url : __ctxPath + "/access/formulateplan/queryAccessPlansPage.f?planType="+"riskResponse",//查询列表url
        	cols:cols,
        	tbarItems:[{
        			btype:'add',
