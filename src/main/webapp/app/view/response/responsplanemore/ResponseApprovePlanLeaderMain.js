Ext.define('FHD.view.response.responsplanemore.ResponseApprovePlanLeaderMain', {
    extend: 'Ext.form.Panel',
    alias: 'widget.responseApprovePlanLeaderMain',
    requires: [
	],
	...
	submit:function(isPass,examineApproveIdea){
		...
		FHD.ajax({//ajax调用
			url : __ctxPath+ '/responseplan/workflow/riskresponseplanapprovalleader.f',
		    params : {
		    ...
		    '->',
		    {
				...
				handler: function () {
					...
					Ext.MessageBox.show({
			    		...
			    		fn : function(btn) {
			    			if (btn == 'yes') {//确认删除
			    				me.submit(me.workPlanApproverSubmitDown.ideaApproval.isPass,
			    							me.workPlanApproverSubmitDown.ideaApproval.getValue());
			    			}
			    		}
			    	});
	            }
