...
Ext.define('FHD.view.report.risk.Top10RiskGrid', {
    extend: 'FHD.ux.GridPanel',
	alias: 'widget.top10riskgrid',
	...
	initComponent: function () {
		...
		Ext.apply(me, {
            ...
            border: false,
            columnLines: true,
            url: __ctxPath + me.listUrl,//'/app/view/report/risk/grid.json',
            extraParams:{
            	level:2,
				schm:me.typeId
            },
            cols: cols,
            tbarItems:['<span id="versionLabel" style="font-size:12px;font-weight:bold;color: #15498b;margin-right:0">当前版本</span>',{
            	btype: 'add',
            	text:'创建新版本',
            	name:'btnCreate',
            	handler:function(){
...
//                }
//            },{
            	iconCls : 'icon-style-go',
            	text: '高级查询',
            	tooltip: '高级查询',
            	handler :me.showSeniorQuery,
            	scope : me
            },{
                text : '导出',
    			tooltip: '导出excel',
	...
	showSeniorQuery : function(){
		var me=this;
		var winform = Ext.create('FHD.view.report.risk.Top10RiskSeniorQueryForm',{
			owner:me,
			typeId:me.typeId
		});
    	var searchWin = Ext.create('FHD.ux.Window',{
				title:'高级查询主窗口',
				height : 300,
				collapsible:false,
				maximizable:true//（是否增加最大化，默认没有）
	    }).show();
	    searchWin.add(winform);
	},
	...
	reloadData : function(version,level,orgIds,query){
		...
		if(orgIds != '' && orgIds != undefined && orgIds != null){
			me.orgIds = orgIds;
		}
		if(query != undefined){
		...
		me.store.proxy.extraParams.version = me.version;//"'"+version+"'";
		...
		me.store.proxy.extraParams.schm = me.typeId;
        me.store.load();
	},
    ...
    exportChart : function(){
		...
		window.location.href = __ctxPath + me.exportUrl + '?version='+version+'&level='+level
								+'&orgIds='+orgIds+'&headerData='+Ext.encode(me.headerDatas);
	},
	
	setVersionLabel:function(txt){
		document.getElementById('versionLabel').innerHTML = txt;
	}
});