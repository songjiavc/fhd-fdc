Ext.define('FHD.view.icm.rectify.RectifyImproveCardPanel', {
    ...
    tbar: {
        action: 'icm_rectifyimprove_card_topbar',
        items: [
	        {
	            text: FHD.locale.get('fhd.common.details'),//基本信息导航按钮
	            iconCls: 'icon-001',
	            action: 'icm_rectifyimprove_details_btn_top',
	            handler: function () {
	        ...
	        '<img src="'+__ctxPath+'/images/icons/show_right.gif">',
	        {
	            text: '范围选择',//范围选择导航按钮
	            iconCls: 'icon-002',
	            action: 'icm_rectifyimprove_range_btn_top',
	            handler: function () {
	            	var rectifyimprovecardpanel = this.up('rectifyimprovecardpanel');
	            	if(rectifyimprovecardpanel){
    ...
    bbar: {
        action: 'icm_rectifyimprove_card_bbar',
        items: [
	        '->', 
	        {
	            action: 'icm_rectifyimprove_card_undo_btn' ,
	        ...
	        {
	            action: 'icm_rectifyimprove_card_pre_btn' ,
	        ...
	        {
	            action: 'icm_rectifyimprove_card_next_btn' ,
	        ...
	        }, 
	        {
	            ...
	            action: 'icm_rectifyimprove_card_finish_btn' ,
	        ...
	        }, 
	        {
	            ...
	            action: 'icm_rectifyimprove_card_submit_btn' ,
    ...
    initComponent: function() {
        ...
        if(me.isWindow){
        	...
        	Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_undo_btn]')[0].hidden = true;
        }else{
        	//Ext.getCmp('icm_rectifyimprove_card_undo_btn').hidden = false;
        	Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_undo_btn]')[0].hidden = false;
        }
    },
    /**
     * 设置导航按钮的事件函数
    ...
    navBtnState:function(){
    	...
    	Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_pre_btn]')[0].setDisabled(!layout.getPrev());
        //Ext.getCmp('icm_rectifyimprove_card_next_btn' ).setDisabled(!layout.getNext());
        Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_next_btn]')[0].setDisabled(!layout.getNext());
    },
    /**
     * 返回按钮事件
     */
    ...
    back:function(){
        ...
        Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_pre_btn]')[0].setDisabled(true);
    ...
    last:function(){
    	...
    	if(me.rectifyimproveform.saveFunc(me)){
    		...
    		Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_next_btn]')[0].setDisabled(true);
    		me.setBtnState(1);
        	...
        	Ext.MessageBox.show({
				...
				fn : function(btn) {
					if (btn == 'yes') {//确认提交
				        ...
				        Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_finish_btn]')[0].setDisabled(true);
				        //Ext.getCmp('icm_rectifyimprove_card_submit_btn').setDisabled(true);
				        Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_submit_btn]')[0].setDisabled(true);
						FHD.ajax({//ajax调用
    ...
    setNavBtnEnable:function(v,first){
    	...
    	if(first){
    		...
    		Ext.ComponentQuery.query('button[action=icm_rectifyimprove_details_btn_top]')[0].setDisabled(v);
    		//Ext.getCmp('icm_rectifyimprove_range_btn_top').setDisabled(v);
    		Ext.ComponentQuery.query('button[action=icm_rectifyimprove_range_btn_top]')[0].setDisabled(v);
    	}else{
    		//新增
    		//Ext.getCmp('icm_rectifyimprove_details_btn_top').setDisabled(first);
    		Ext.ComponentQuery.query('button[action=icm_rectifyimprove_details_btn_top]')[0].setDisabled(first);
    		//Ext.getCmp('icm_rectifyimprove_range_btn_top').setDisabled(v);
    		Ext.ComponentQuery.query('button[action=icm_rectifyimprove_range_btn_top]')[0].setDisabled(v);
    	}
    	me.setBtnState(0);
    ...
    setBtnState: function (index) {
        var k = 0;
        var topbar = Ext.ComponentQuery.query('toolbar[action=icm_rectifyimprove_card_topbar]')[0];
    ...
    setFirstItemFoucs:function(disable){
        ...
        Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_pre_btn]')[0].setDisabled(disable);
        //Ext.getCmp('icm_rectifyimprove_card_submit_btn').setDisabled(disable);
        Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_submit_btn]')[0].setDisabled(disable);
    },
    /**
     * 设置上一步和下一步按钮的状态
     */
    navBtnState:function(){
        ...
        Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_finish_btn]')[0].setDisabled(!layout.getNext());
    	if(me.improveId){
        	//修改
        	//Ext.getCmp('icm_rectifyimprove_card_submit_btn' ).setDisabled(!layout.getPrev());
        	Ext.ComponentQuery.query('button[action=icm_rectifyimprove_card_submit_btn]')[0].setDisabled(!layout.getPrev());
        }else{
        	//添加
