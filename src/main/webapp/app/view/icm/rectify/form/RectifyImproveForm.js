...
Ext.define('FHD.view.icm.rectify.form.RectifyImproveForm', {
	...
	saveFunc:function(cardpanel){
		...
		FHD.ajax({
             url: __ctxPath+'/icm/rectify/validateImprove.f',
             params: {
             	 id: me.getForm().getValues().id,
                 code: me.getForm().getValues().code
             },
             callback: function (data) {
             	 var formValues = me.getForm().getValues();
                 if(data.flagStr == "codeRepeat") {
                	 Ext.MessageBox.alert(FHD.locale.get('fhd.common.prompt'), '整改计划编号重复!');
                	 return;
                 }
