Ext.define('FHD.view.icm.assess.AssessPlanCardPanel', {
    ...
    tbar: {
        name: 'icm_assessplan_card_topbar',
        items: [
	        {
	            text: FHD.locale.get('fhd.common.details'),//基本信息导航按钮
	            iconCls: 'icon-001',
	            name: 'icm_assessplan_card_details_btn_top',
	            handler: function () {
	        ...
	        '<img src="'+__ctxPath+'/images/icons/show_right.gif">',
	        {
	            text: '范围选择',//范围选择导航按钮
	            iconCls: 'icon-002',
	            name: 'icm_assessplan_card_range_btn_top',
	            handler: function () {
	            	var assessplancardpanel = this.up('assessplancardpanel');
	            	if(assessplancardpanel){
    ...
    bbar: {
        ...
        items: [
	        ...
	        },
	        {
	            ...
	            name: 'icm_assessplan_card_pre_btn' ,
	        ...
	        }, 
	        {
	            ...
	            name: 'icm_assessplan_card_next_btn' ,
	        ...
	        }, 
	        {
	            ...
	            name: 'icm_assessplan_card_finish_btn' ,
	        ...
	        }, 
	        {
	            ...
	            name: 'icm_assessplan_card_submit_btn' ,
    ...
    initComponent: function() {
        ...
        if(me.executionId){
    		me.down('[name=icm_assessplan_card_undo_btn]').hidden=true;
    		me.setNavBtnEnable(false,true);
        }else{
    		me.down('[name=icm_assessplan_card_undo_btn]').hidden=false;
    	}
    },
    /**
     * 返回按钮事件
    ...
    last:function(){
    	...
    	if(me.assessplanform.saveFunc(me)){
    		me.down('[name=icm_assessplan_card_range_btn_top]').setDisabled(false);
    		me.editflag=true;
    		me.setBtnState(1);
    		if(me.businessId){
				...
				if (data) {
					Ext.MessageBox.show({
			            ...
			            fn: function (btn) {
			                if (btn == 'yes') {
            					//所有按钮不可用，提交成功后跳转到列表
            			    	me.down('[name=icm_assessplan_card_pre_btn]').setDisabled(true);
            			        me.down('[name=icm_assessplan_card_next_btn]').setDisabled(true);
            			        me.down('[name=icm_assessplan_card_finish_btn]').setDisabled(true);
            			        me.down('[name=icm_assessplan_card_submit_btn]').setDisabled(true);
    ...
    navBtnState:function(){
    	...
    	me.down('[name=icm_assessplan_card_pre_btn]' ).setDisabled(!layout.getPrev());
        me.down('[name=icm_assessplan_card_next_btn]' ).setDisabled(!layout.getNext());
        me.down('[name=icm_assessplan_card_submit_btn]' ).setDisabled(!layout.getPrev());
    },
    /**
     * 设置导航按钮的选中或不选中状态
    ...
    setBtnState: function (index) {
        ...
        var k = 0;
        var topbar = me.down('[name=icm_assessplan_card_topbar]');
    ...
    setNavBtnEnable:function(v,first){
    	...
    	if(first){
    		//修改
    		me.down('[name=icm_assessplan_card_details_btn_top]').setDisabled(v);
    		me.down('[name=icm_assessplan_card_range_btn_top]').setDisabled(v);
    	}else{
    		//新增
    		me.down('[name=icm_assessplan_card_details_btn_top]').setDisabled(first);
    		me.down('[name=icm_assessplan_card_range_btn_top]').setDisabled(v);
    	}
    	me.setBtnState(0);
    	me.navBtnHandler(me,0);
