...
Ext.define('FHD.view.icm.assess.form.AssessPlanForm',{
	...
	initComponent : function() {
		...
		//组长
		me.assessPlanGroup = Ext.create('Ext.ux.form.OrgEmpSelect', {
		...
		//组成员
		me.assessPlanGroupPers = Ext.create('Ext.ux.form.OrgEmpSelect', {
			type: 'emp',
	...
	validateDate:function(date1,date2){//验证时间
	 ...
	 submitData:function(cardpanel){
    	...
    	if(vobj.groupLeaderId == undefined || vobj.groupLeaderId =='' || vobj.groupLeaderId =='[]'){
    		FHD.notification('组长必选!',FHD.locale.get('fhd.common.prompt'));
    		return false;
    	}
        ...
        if(me.assessPlanType.getValue()=='ca_assessment_etype_other'){//他评
        	if(me.assessPlanGroupPers.getValue()==undefined || me.assessPlanGroupPers.getValue()=='' || me.assessPlanGroupPers.getValue()=='[]'){
        		FHD.notification('组员必选!',FHD.locale.get('fhd.common.prompt'));
        		return false;
        	}
        }
        /*
    	if(me.assessPlanType.getValue()=='ca_assessment_etype_other'){//他评
        	if(me.dxis.getValue()==undefined || me.dxis.getValue()=='' || me.dxis.getValue()=='[]'){
        		FHD.notification('组员必选!',FHD.locale.get('fhd.common.prompt'));
        		return false;
        	}
        }*/
        if(vobj.planStartDate!=''&&vobj.planEndDate!=''){
        	 if(!me.validateDate(vobj.planStartDate,vobj.planEndDate)){
        		FHD.notification('计划开始时间不能大于结束时间!',FHD.locale.get('fhd.common.prompt'));
             	return false;
             }
        }
        ...
        if(vobj.targetStartDate!=''&&vobj.targetEndDate!=''){
        	if(!me.validateDate(vobj.targetStartDate,vobj.targetEndDate)){
        		FHD.notification('评价开始时间不能大于结束时间!',FHD.locale.get('fhd.common.prompt'));
            	return false;
            }
        }
		...
		FHD.ajax({
	        ...
	        callback: function (data) {
	        	if (data.flagStr=='nameRepeat') {
	        		FHD.notification('评价计划名称重复!',FHD.locale.get('fhd.common.prompt'));
	        		flag = false;
	        		return false;
	        	}
	        	if (data.flagStr=='codeRepeat') {
	        		FHD.notification('评价计划编号重复!',FHD.locale.get('fhd.common.prompt'));
	        		flag = false;
	        		return false;
	        	}
		...
		if(editflag){
			...
			me.getForm().load({
		    	 ...
		    	 success: function (form, action) {
		    		 //手动设置控件的值
		    		 if(action.result.data.groupLeaderId){
			    		 me.assessPlanGroup.setValues(Ext.JSON.decode(action.result.data.groupLeaderId));
		    		 }
		    		 if(action.result.data.groupPersId){
		    			 me.assessPlanGroupPers.setValues(Ext.JSON.decode(action.result.data.groupPersId));
		    		 }
		    		 return true;
		    	 },
