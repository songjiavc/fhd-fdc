Ext.define('FHD.view.icm.assess.component.AssessPlanSelectorCardPanel', {
    ...
    tbar: {
        name: 'icm_assessplan_selector_card_topbar',
        items: [
	        {
	            text: FHD.locale.get('fhd.common.details'),//基本信息导航按钮
	            name: 'icm_assessplan_selector_card_details_btn_top',
	            iconCls: 'icon-001',
	            handler: function () {
	        ...
	        {
	            text: '范围选择',//范围选择导航按钮
	            name: 'icm_assessplan_selector_card_range_btn_top',
	            iconCls: 'icon-002',
	            handler: function () {
	            	var assessplanselectorcardpanel = this.up('assessplanselectorcardpanel');
	            	if(assessplanselectorcardpanel){
    ...
    bbar: {
    	name: 'icm_assessplan_selector_card_bbar',
        items: [
	        '->', 
	        {
	            text: FHD.locale.get("fhd.strategymap.strategymapmgr.form.back"),//上一步按钮
	            name: 'icm_assessplan_selector_card_pre_btn' ,
	        ...
	        }, 
	        {
	            ...
	            name: 'icm_assessplan_selector_card_next_btn' ,
	        ...
	        }, 
	        {
	            ...
	            name: 'icm_assessplan_selector_card_finish_btn' ,
	        ...
	        }, 
	        {
	            ...
	            name: 'icm_assessplan_selector_card_submit_btn' ,
    ...
    last:function(){
    	...
    	if(me.assessplanform.saveFunc(me)){
    		me.down('[name=icm_assessplan_selector_card_range_btn_top]').setDisabled(false);
    		me.setBtnState(1);
    		if(me.businessId){
    ...
    submit:function(){
		...
		FHD.ajax({
			...
			callback : function(data) {
				if (data) {
					//所有按钮不可用，提交成功后跳转到列表
			    	me.down('[name=icm_assessplan_selector_card_pre_btn]').setDisabled(true);
			        me.down('[name=icm_assessplan_selector_card_next_btn]').setDisabled(true);
			        me.down('[name=icm_assessplan_selector_card_finish_btn]').setDisabled(true);
			        me.down('[name=icm_assessplan_selector_card_submit_btn]').setDisabled(true);
    ...
    navBtnState:function(){
    	...
    	me.down('[name=icm_assessplan_selector_card_pre_btn]').setDisabled(!layout.getPrev());
        me.down('[name=icm_assessplan_selector_card_next_btn]').setDisabled(!layout.getNext());
        me.down('[name=icm_assessplan_selector_card_submit_btn]').setDisabled(!layout.getPrev());
    },
    /**
     * 设置导航按钮的选中或不选中状态
    ...
    setBtnState: function (index) {
        ...
        var k = 0;
        var topbar = me.down('[name=icm_assessplan_selector_card_topbar]');
    ...
    setNavBtnEnable:function(v,first){
    	...
    	if(first){
    		//修改
    		me.down('[name=icm_assessplan_selector_card_details_btn_top]').setDisabled(v);
    		me.down('[name=icm_assessplan_selector_card_range_btn_top]').setDisabled(v);
    	}else{
    		//新增
    		me.down('[name=icm_assessplan_selector_card_details_btn_top]').setDisabled(first);
    		me.down('[name=icm_assessplan_selector_card_range_btn_top]').setDisabled(v);
    	}
    	me.setBtnState(0);
    	me.navBtnHandler(me,0);
