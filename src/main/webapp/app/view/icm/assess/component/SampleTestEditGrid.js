 ...
 * */
 Ext.define('FHD.view.icm.assess.component.SampleTestEditGrid',{
	...
	initComponent:function(){
    	...
    	me.cols=[
		    {header:'状态', dataIndex: 'type', sortable: false,width:45},
		    ...
		    {header:'是否有效<font color=red>*</font>', dataIndex: 'isQualified', sortable: false,width:80,emptyCellText:'<font color="#808080">请选择</font>',
		    	...
		    	renderer:function(value,metaData,record,colIndex,store,view) {
		    		...
		    		if(record){
						...
						if(value){
							...
							metaData.tdAttr = 'data-qtip="样本“是否有效”,必填" style="background-color:#FFFBE6"';
						}
					}
		    	}
		    },
		    {header:'说明', dataIndex: 'comment',sortable: false,flex:2,editor: {allowBlank: true},emptyCellText:'<font color="#808080">请填写</font>',
		    	renderer:function(value,metaData,record,colIndex,store,view) { 
					if(value){
						metaData.tdAttr = 'data-qtip="'+value+'" style="background-color:#FFFBE6"';
						return value;  
					}else{
						metaData.tdAttr = 'data-qtip="样本“是否有效”为“无效”的样本，“说明”必填" style="background-color:#FFFBE6"';
					}
				}
		    },
		    {header:'影响程度', dataIndex: 'impact',sortable: false,width:80,
		    	...
		    	renderer:function(value,metaData,record,colIndex,store,view) { 
					if(value){
						...
						metaData.tdAttr = 'data-qtip="样本“是否有效”为“无效”的样本，“影响程度”必填，为1~5之间的数" style="background-color:#FFFBE6"';
					}
				}
		    },
		    {header:'补充的源样本', dataIndex: 'sourceSample',sortable: false,flex:1},
		    {header:'附件', dataIndex: 'file',sortable: false,width:135,
			    renderer : function(value, metaData, record, colIndex, store, view) { 
					if(value){
			    		...
			    		metaData.tdAttr = 'data-qtip="无效的样本必须上传附件"'; 
    		...
    		Ext.MessageBox.show({
                ...
                fn: function (btn) {
                    if (btn == 'yes') {
                        ...
                        if(flag){
                        	FHD.notification('"自动"状态的样本不允许删除!',FHD.locale.get('fhd.common.prompt'));
                        	return;
                        }
