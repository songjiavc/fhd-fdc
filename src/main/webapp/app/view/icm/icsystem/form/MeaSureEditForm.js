...
Ext.define('FHD.view.icm.icsystem.form.MeaSureEditForm', {
    ...
    addComponent: function () {
		...
		me.mainfieldset = Ext.widget('fieldset', {
			...
			title: '<input id="c1c"  type="checkbox" onclick="Ext.getCmp(\''+me.id+'\').select1(this.checked)"/></img>控制措施信息'
		});
	    //基本信息fieldset
...
//	        me.add(me.basicinfofieldset);
		...
		me.pointNote = new Ext.widget('combobox',{
			fieldLabel : '对应的流程节点',
			name : 'pointNote',
	        ...
	        me.load({
	            ...
	            success: function (form, action) {
	            	...
	            	if(action.result.data.meaSureorgId){
	            		me.noteDepart.setValues(Ext.JSON.decode(action.result.data.meaSureorgId));
	            	}
	            	if(action.result.data.meaSureempId){
	            		me.noteradio.setValues(Ext.JSON.decode(action.result.data.meaSureempId));
	            	}
	                return true;
	            }
	        });
