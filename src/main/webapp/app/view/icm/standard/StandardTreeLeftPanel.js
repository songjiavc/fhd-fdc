...
Ext.define('FHD.view.icm.standard.StandardTreeLeftPanel', {
    ...
    selectId:null,
    url: __ctxPath + '/standard/standardTree/findStandardTreeLoader.f',
    rootVisible: true,
	root:{
		text : '内部控制应用指引',
		id : '',
            ...
            fn: function (btn) {
                if (btn == 'yes') {
                    FHD.ajax({
                        ...
                        url : __ctxPath+ '/standard/standardTree/removeStandards.f',
                        callback: function (ret) {
                            if (ret && ret.result) {
                                if (ret.result == "cascade") {
                                    FHD.notification(FHD.locale.get("fhd.kpi.kpi.prompt.categorycasdes"), FHD.locale.get('fhd.common.prompt'));
    ...
    levelHandler : function (id,rec) {
        ...
        standardtab.standardedit.controlType = 'treeEdit';
	...
	contextItemMenuFun:function(view, rec, node, index, e){
       ...
       //添加下级菜单
        var subLevel = {
            iconCls: 'icon-add',
            text: '添加下级',//FHD.locale.get('fhd.strategymap.strategymapmgr.subLevel'),
            handler: function () {
            	me.currentNode = rec;
    ...
    currentNodeClick:function(){
		...
		paramObj.controlType='addTree';
		standardedit.initParam(paramObj);
    	var nodeItem = me.getSelectionModel().getLastSelected();//.selected.items;
        if (nodeItem) {
            me.selectId = nodeItem.data.id;
        }
        standardtab.standardedit.controlType = 'treeEdit';
        if(me.selectId==''){
			...
			form.reset();
			form.setValues({'upName':nodeItem.parentNode.data.text});
