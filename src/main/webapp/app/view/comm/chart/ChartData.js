...
Ext.define('FHD.view.comm.chart.ChartData', {
		...
		if(isNaN(value1)){
			...
			if(!isNaN(value2)){
				if(valueColSumMethod=="multiply"){
					...
					value=(value1*weight1+value2*weight2)/(weight1+weight2);
	...
	getChartTotal:function(){
		...
		me.chartTotal=Math.round(me.chartTotal*100)/100
	...
	getChartSum:function(){
		...
		if(me.chartSum==null||!me.compareCols(me.chartSum.cols,me.cols)){
			...
			var n=me.chartSum.sets.length;
			for (var i = 0; i < n; i++) {
				me.chartSum.sets[i].value=Math.round(me.chartSum.sets[i].value*100)/100
			}
		}
		return me.chartSum;
	},
	...
	getChartDetail:function(){
		...
		var n=me.chartDetail.sets.length;
		for (var i = 0; i < n; i++) {
			var n1=me.chartDetail.sets[i].sets.length;
			for (var j = 0; j < n1; j++) {
				me.chartDetail.sets[i].sets[j].value=Math.round(me.chartDetail.sets[i].sets[j].value*100)/100
			}
		}
	...
	baseDataToHGridData:function(){
		...
		var n=me.hGridData.datas.length;
		for (var i = 0; i < n; i++) {
			var n1=me.hGridData.multiColumns.length;
			for (var j = 0; j < n1; j++) {
				if(me.hGridData.multiColumns[j].dataIndex!=xColDataIndex&&!isNaN(me.hGridData.datas[i][me.hGridData.multiColumns[j].dataIndex])){
					me.hGridData.datas[i][me.hGridData.multiColumns[j].dataIndex]=Math.round(me.hGridData.datas[i][me.hGridData.multiColumns[j].dataIndex]*100)/100
				}
			}
		}
	},
	...
	baseDataToVGridData:function(){
		...
		var n=me.vGridData.datas.length;
		for (var i = 0; i < n; i++) {
			me.vGridData.datas[i][valueColDataIndex]=Math.round(me.vGridData.datas[i][valueColDataIndex]*100)/100
		}
	},
	/**
	 * 初始化方法
