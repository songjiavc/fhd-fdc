Ext.define('FHD.view.sys.dict.DictTypeTreePanel', {
    extend: 'FHD.ux.TreePanel',
    alias: 'widget.dicttypetreepanel',
	rootVisible : true,
    url:__ctxPath +"/sys/dict/dictType/getTreeRoot.f",
    treeRootUrl:__ctxPath +"/sys/dict/dictType/treeLoader.f",
	removeUrl:__ctxPath +'/sys/auth/auth/removeById.f',
	nodeMenu:null,
	selectNode:null,
	selectNodeId:null,
	treeRoot:null,
	b_reloadData:function(){
    },
   	b_selectNode:function(node){
			...
			listeners : {
				load: function (store, records) {
	            	...
	            	if(me.selectNode.data.id==""&&record!=me.selectNode){
	            		Ext.MessageBox.confirm('警告', "新增类型未保存，是否要放弃？", function showResult(btn){
		               		if (btn == 'yes') {//确认删除
