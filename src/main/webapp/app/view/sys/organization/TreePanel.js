...
Ext.define('FHD.view.sys.organization.TreePanel', {
    ...
    itemclickTree : function(re){
		...
		var posiEmpGrid = Ext.getCmp('positionempGridPanel');
		if("jg" == re.data.type){
			orgEditPanel.idfiled.setValue(me.currentNode.data.id);
			cardPanel.layout.setActiveItem(tabPanel);
			orgGridPanel.isAdd = false;
			if(tabPanel.getActiveTab()==tabPanel.items.items[0]){
				orgGridPanel.store.proxy.url = orgGridPanel.queryUrl;//动态赋给机构列表url
	  			orgGridPanel.store.proxy.extraParams.orgIds = me.currentNode.data.id;
	  			orgGridPanel.store.proxy.extraParams.positionIds = null;
	  			orgGridPanel.store.load();
			}else if(tabPanel.getActiveTab()==tabPanel.items.items[1]){
				positionGridPanel.store.proxy.url = positionGridPanel.queryUrl;
				...
				empGridPanel.store.proxy.url = empGridPanel.queryUrl;
  				...
  				empGridPanel.store.proxy.extraParams.positionIds = null;
  				empGridPanel.store.load();
			}else if(tabPanel.getActiveTab()==tabPanel.items.items[3]){
			...
			cardPanel.layout.setActiveItem(positionTabPanel);
			positionGridPanel.isAdd=false;
			if(positionTabPanel.getActiveTab()==positionTabPanel.items.items[0]){
				posiEmpGrid.store.proxy.url = posiEmpGrid.queryUrl;
				posiEmpGrid.store.proxy.extraParams.orgIds = null;
                    						...
                    						FHD.ajax({//ajax调用
                        						...
                        						callback : function(data) {
                        							if (data) {//删除成功！
                        								...
                        								FHD.notification('有子集不能删除',FHD.locale.get('fhd.common.prompt'));
                         						}
                        						}
                        					});
                    					}else{
    	...
    	Ext.apply(me, {
    		...
    		split: true,
          	...
          	singleExpand: false,
   			...
   			root:root,
   			listeners : {
   				load: function (store, records,successful) { //默认选择首节点
   					var selectedNode;
   					if(me.searchField.lastValue){
   						if(successful[0]){
   							if(successful[0].data.text.search(me.searchField.lastValue) != -1){
   								me.getSelectionModel().select(successful[0]);//默认选择首节点
			            		me.itemclickTree(successful[0]);
   							}else if(records.childNodes.length>0){
   								var isNode;
								Ext.each(records.childNodes,function(item){
									if(item.data.text.search(me.searchField.lastValue) != -1){
								  		isNode = item;
								  	}
								});
								if(isNode){
									me.getSelectionModel().select(isNode);//默认选择首节点
			            			me.itemclickTree(isNode);
								}else{
									successful[0].expand();
								}
   							}else{
