    ...
    initComponent: function() {
    	...
    	me.gridColums =[
    		...
    		{header: FHD.locale.get('fhd.sys.orgstructure.org.parentorgname') ,
				dataIndex: 'parentNameSeq',sortable: true,flex : 1,renderer:function(value) {
					var showValue="";
					if(value!=null){
						var values=value.split(">");
						showValue=values.join("<img height='12' width='12' src='"+__ctxPath+"/images/page-next-disabled-bottom.gif?a=3'></imp>");
					}
					return showValue;
				}
			},//上级机构
    	...
    	me.tbar =[//机构菜单项
    	           {text : FHD.locale.get('fhd.common.add'),
    	        	   iconCls: 'icon-add',id:'orgadd'+variable, handler:function(){me.edit('add', me, false,false);}, scope : this},'-',
    	...
    	if(id=='add'|| id=='orgAdd'){//新增机构
    		...
    		if(id == 'edit'){
    			if(!isMenu){
    				var selections = me.getSelectionModel().getSelection();//得到选中的记录
    	    	     ...
    	    	     if (length >= 2) {//判断是否多选
    	    	    	...
    	    	    	orgEditPanel.idfiled.setValue(selection.data.id);
    	    	    	cardPanel.getLayout().setActiveItem(tabPanel);
    	    	    	tabPanel.setActiveTab(3);
    	    	     }
    			}else{
    				orgEditPanel.idfiled.setValue(treepanel.currentNode.data.id);
    			}
    			cardPanel.getLayout().setActiveItem(tabPanel);
		    	tabPanel.setActiveTab(3);
    		}else{
    			if(!isMenu){
    				...
    				orgEditPanel.idfiled.setValue(orgtreeId);
    			}
    		}
    		orgEditPanel.load();
    	}
