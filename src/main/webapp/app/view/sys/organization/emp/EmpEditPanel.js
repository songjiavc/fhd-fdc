...
Ext.define('FHD.view.sys.organization.emp.EmpEditPanel', {
    	...
    	if(typeof(me.empgridId) != 'undefined') {
    		empEditPanel.form.load({
    	        ...
    	        success:function(form,action){
    	        	...
    	        	if(action.result.data.orgStrMain){
						me.oSysOrgMain.setValues(Ext.JSON.decode(action.result.data.orgStrMain));
					}
    	        	//用户锁定状态
    ...
    //新增员工默认主责部门赋值
    mainOrgLoad : function(orgId){
    	var me = this;
    		me.form.load({
    	        url:'sys/organization/findmainorgbyorgnodeid.f',
    	        params:{
    	        	orgId: orgId
        		},
    	        failure:function(form,action) {
    ...
    initComponent: function () {
        ...
        /***attribute start***/
        	...
        	//主责部门
        	me.oSysOrgMain = Ext.create('Ext.ux.form.OrgEmpSelect',{
...
//        	});
        	...
        	//密码
        	me.password=Ext.create('Ext.form.TextField', {
        	...
        	//用户状态
        	 me.userStatus = Ext.create('Ext.form.field.ComboBox', {
        		    store: statusStore,
        		    fieldLabel: "用户状态",
        		    emptyText:FHD.locale.get('fhd.common.pleaseSelect'),//默认为空时的提示   
        		    editable:false,
        	...
        	//锁定状态
        	 var lockState = Ext.create('Ext.form.RadioGroup', {
        		 ...
        		 margin: '7 30 3 30',
        	        vertical: true,
        	        items: [
        	            { boxLabel: "锁定", name: 'lockstate', inputValue: '1',checked:false,id:'lockstateYes'},
        	            { boxLabel: "未锁定", name: 'lockstate', inputValue: '0',checked:true,id:'lockstateNo'}]
        	  });
    	...
    	function returnTab(){
    		...
    		var tabPanel = Ext.getCmp('tabPanel');
    		var rightPanel = Ext.getCmp('rightPanel');
    		tabPanel.orgEditPanel.getForm().reset();//重置机构基本信息表单
    		rightPanel.items.items[0].setHeight(20);
    		cardPanel.getLayout().setActiveItem(tabPanel);
        ...
        Ext.applyIf(me, {
            ...
            items: [{	
				...
				title: '员工信息',
				items:[empCode, empName, empDuty,oTel, ozipCode,me.oSysOrgMain, oEmail, empStatus
				         					//oSysOrgs辅助部门隐藏
				]
			},
			{	
				...
				title: '用户信息',
				items:[userName,  me.password, me.userStatus, lastLoginTime,
