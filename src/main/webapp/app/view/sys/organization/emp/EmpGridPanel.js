...
//    	me.gridqueryUrl = 'sys/organization/queryOrgPage.f';//查询所有机构
    	...
    	me.gridColums =[
    	             	...
    	             	{header: '角色' ,
            	          	dataIndex: 'roleName',sortable: true,flex : 1,
    	             	...
    	             	{header: FHD.locale.get('fhd.common.status') ,
    	             		...
    	             		renderer:function(dataIndex) { 
    	          			  if(dataIndex == "1"){
    	          				  ...
    	          				  return "<font style='color:red'>注销</font>";
    	          			  }
    	             	}},
    ...
    userAuthrity : function(me){//用户授权方法
    	...
    	var length = selections.length;
	     if (length >= 2) {//判断是否多选
	          FHD.notification('只能同时给一位用户授权',FHD.locale.get('fhd.common.prompt'));
	          return;
	      }else{
    ...
    empedit : function(id, me){//员工新增方法
    	...
    	var empEditPanel_mainorg = empEditPanel.oSysOrgMain;
    	...
    	if(id=='add'){
    		...
    		if(treePanel.currentNode && 'jg'==treePanel.currentNode.data.type){
    			empEditPanel.mainOrgLoad(treePanel.currentNode.data.id);
    		}else{
    			if('jg'==treePanel.currentNode.parentNode.data.type){
    				empEditPanel.mainOrgLoad(treePanel.currentNode.parentNode.data.id);
    			}
    		}
    		empEditPanel.password.setValue('111111');//密码默认
    		empEditPanel.userStatus.setValue('1');//用户状态默认
    		me.isAdd = true;
    	}else{
    		 var length = selections.length;
    	     if (length >= 2) {//判断是否多选
    ...
    setstatus : function(me){//设置按钮可用状态
    	...
    	Ext.getCmp('emproletabpanel').setDisabled(me.getSelectionModel().getSelection().length != 1);//分配角色
    	Ext.getCmp('emproleposition').setDisabled(me.getSelectionModel().getSelection().length === 0);
    	/*Ext.getCmp('yhsqtabpanel').setDisabled(me.getSelectionModel().getSelection().length === 0);//用户授权--按钮暂不可用
    			...
    			if (btn == 'yes') {//确认重置
        				...
        				FHD.ajax({//ajax调用
        					...
        					callback : function(data){
        						if(data){//重置成功！
        							FHD.notification('重置成功',FHD.locale.get('fhd.common.prompt'));
        						}
        					}
        				});
    				}
