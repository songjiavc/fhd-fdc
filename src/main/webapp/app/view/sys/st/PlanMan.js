...
Ext.define('FHD.view.sys.st.PlanMan', {
    ...
    exeFunUrl: __ctxPath + '/sys/st/exeFun.f',
    // 初始化方法
    initComponent: function() {
    	...
    	me.formwindow;
    	me.viewwindow;
    	me.planDelUrl = __ctxPath + '/sys/st/removePlanBatch.f'; //删除url
    	me.planQueryUrl = __ctxPath + '/sys/st/queryPlanPage.f'; //查询分页url
    	me.planStatusUrl = __ctxPath + '/sys/st/mergePlanBatch.f';//启用/停用url
    	me.planViewUrl = __ctxPath + '/sys/st/viewPlan.f';//查看url
    	me.logGridUrl = __ctxPath + '/pages/sys/st/planDealLogMan.jsp';
    	me.gridColums =[
     		//{header:'',dataIndex: 'planEmpId', sortable: true, flex : 1,hidden : true},
     		{dataIndex: 'value',invisible:true},
     		...
     		{header: FHD.locale.get('fhd.sys.planMan.operate') ,dataIndex: 'statusName', sortable: true, flex : 1,
     			renderer:function(value,metaData,record,colIndex,store,view) {
	     			if(record.get('status') == 1){
	     				//启用
	     				return "<a href=\"javascript:void(0);\" onclick=\"Ext.getCmp('planManId').runFun('" + record.get('value') + "', Ext.getCmp('planManId'))\">" + 
	     						"<image src='images/icons/move_right.gif' width='15' height='15' title='执行'/></a>&nbsp;&nbsp;&nbsp;" + 
	     						"<a href=\"javascript:void(0);\" onclick=\"Ext.getCmp('planManId').stop('" + record.get('id') + "', Ext.getCmp('planManId'))\">" + 
	     						"<image src='images/icons/stop.gif' width='15' height='15' title='停用'/></a>&nbsp;&nbsp;&nbsp;" + 
	     						"<a href=\"javascript:void(0);\" onclick=\"Ext.getCmp('planManId').viewLog('" + record.get('id') + "', Ext.getCmp('planManId'))\">" +
	     						"<image src='images/icons/table_go.png' width='15' height='15' title='日志'/></a>";
	     			}else{
	     				//停用
	     				return "<a href=\"javascript:void(0);\" onclick=\"Ext.getCmp('planManId').runFun('" + record.get('value') + "', Ext.getCmp('planManId'))\">" + 
	     						"<image src='images/icons/move_right.gif' width='15' height='15' title='执行'/></a>&nbsp;&nbsp;&nbsp;" + 
	     						"<a href=\"javascript:void(0);\" onclick=\"Ext.getCmp('planManId').start('" + record.get('id') + "', Ext.getCmp('planManId'))\">" + 
	     						"<image src='images/icons/start.gif' width='15' height='15' title='启用'/></a>&nbsp;&nbsp;&nbsp;" + 
	     						"<a href=\"javascript:void(0);\" onclick=\"Ext.getCmp('planManId').viewLog('" + record.get('id') + "', Ext.getCmp('planManId'))\">" +
	     						"<image src='images/icons/table_go.png' width='15' height='15' title='日志'/></a>";
	...
	edit : function(param){
		...
		me.formwindow.add(me.planEdit);
		
		if(param == 'planMan_add'){
			me.isAdd = true;
	...
	setstatus : function(){
		...
		me.grid.down('[name=planMan_edit]').setDisabled(me.grid.getSelectionModel().getSelection().length === 0);
		me.grid.down('[name=planMan_del]').setDisabled(me.grid.getSelectionModel().getSelection().length === 0);
		me.grid.down('[name=planMan_start]').setDisabled(me.grid.getSelectionModel().getSelection().length === 0);
		me.grid.down('[name=planMan_stop]').setDisabled(me.grid.getSelectionModel().getSelection().length === 0);
	},
					...
					FHD.ajax({
						...
						callback : function(data){
							if(data){
								...
								FHD.alert(FHD.locale.get('fhd.common.operateFailure'));
							}
						}
					});
				}
