...
Ext.define('FHD.view.sys.st.PlanEdit', {
    ...
    mixins: {
    	planEditFind: 'FHD.view.sys.st.PlanEditFind',
    	planSelect:'FHD.view.sys.st.PlanSelect'
    },
    ...
    initComponent: function() {
    	...
    	var queryUrl = __ctxPath + '/sys/st/findPlanById.f';//查询分页
    	var planEdit_saveUrl = __ctxPath + '/sys/st/savePlan.f';//保存
    	var queryTempListUrl = __ctxPath + '/sys/st/findTempAll.f';//选择模版下拉菜单
    	//var queryEntryByDictTypeIdUrl = __ctxPath + '/sys/st/findDictEntryByDictTypeId.f?dictTypeId=st_trigger_type';//开始任务下拉菜单
    	var dayByMonthStoreUrl = __ctxPath + '/sys/st/dayByMonthStore.f';//对按天的月数
    	var monthStore = __ctxPath + '/sys/st/monthStore.f';//月数
    	var re;
    	...
    	var vLack = 4;//任务名称不能少于字符数
    	var planSelect = me.mixins.planSelect;
    	...
    	tempComboBox = Ext.create('Ext.form.field.ComboBox', {
    	    store: tempStore,
    	    fieldLabel: '执行操作'+'<font color=red>*</font>',
    						...
    						items:[
								{
								   ...
								   columns: 2,
								   items: [
								       {boxLabel: FHD.locale.get('fhd.sys.planEdit.timeScope'), name: "timeType", inputValue: "1" , checked:"true"},
								       {boxLabel: FHD.locale.get('fhd.sys.planEdit.timeForever'), name: "timeType", inputValue: "0" }
								   ],
								   listeners: {
									   click: {
								          element: 'el',
								          fn: function(){
								        	  var timeTypeId = me.down('[name=timeTypeId]');
								        	  for(var i = 0; i < timeTypeId.items.length; i++){
								        		  if(timeTypeId.items.items[i].checked){
								        			  if(timeTypeId.items.items[i].inputValue == '1'){
								        				  Ext.getCmp('sf2').show();
								        				  //me.down('[name=sf2]').show();
								        				  //开始日期
								        				  Ext.getCmp('startTimeid').allowBlank = false;
								        				  //结束日期
								        				  Ext.getCmp('endTimeid').allowBlank = false;
								        			  }else if(timeTypeId.items.items[i].inputValue == '0'){
								        				  Ext.getCmp('sf2').hide();
								        				  //me.down('[name=sf2]').hide();
								        				  //开始日期
    						...
    						items:[{
	                            ...
	                            validator:function(value){
									if(value != ''){
										...
										}else if(Ext.getCmp('endTimeid').value && Ext.getCmp('startTimeid').value > Ext.getCmp('endTimeid').value){
											return FHD.locale.get('fhd.sys.planEdit.startTime') + 
							...
							},
							{
					        	...
					        	validator:function(value){
		                            if(value != ''){
			                            ...
			                            }else if(Ext.getCmp('endTimeid').value == Ext.getCmp('startTimeid').value){
			                            	return FHD.locale.get('fhd.sys.planEdit.entTime') + 
	                            				FHD.locale.get('fhd.sys.planEdit.eqOrLe') + FHD.locale.get('fhd.sys.planEdit.startTime');
	                            		}else{
				                            return true;
    	...
    	triggerTypeComboBox.store.on('beforeload', function (store, options) {
    		if(!me.planManId.isAdd){
    			me.mixins.planEditFind.findPlanById(formPanel, 
					me.planManId.planId, 
					queryUrl, 
					triggerTypeComboBox, 
