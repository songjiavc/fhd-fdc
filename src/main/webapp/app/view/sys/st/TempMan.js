...
Ext.define('FHD.view.sys.st.TempMan', {
    ...
    initComponent: function() {
    	var me = this;
    	me.queryUrl = __ctxPath + '/sys/st/findDictEntryBySome.f?dictEntryId=st_temp_category';
    	me.tempMan_saveUrl = __ctxPath + '/sys/st/saveTemp.f';
    	me.editor;
    	me.dictEntryId='';
    	me.tempGridqueryUrl = __ctxPath + '/sys/st/findDictEntryByType.f';
    	me.formUrl = __ctxPath +'/sys/st/findTempByCategory.f';
    	...
    	me.taskTree = Ext.create('FHD.ux.TreePanel',{
    		...
    		listeners : {
     			'itemclick' : function(view,re){
    				...
    				me.tempGrid.store.proxy.url = me.tempGridqueryUrl;
    				me.tempGrid.store.proxy.extraParams.dictTypeId = 'plan_temp_parameter';
    				me.tempGrid.store.load();
					me.loadForm(me);
      			},
      			load: function (store, records) {
      				var selectedNode;
      				if(records.childNodes){
      					selectedNode = records.childNodes[0];
      					me.taskTree.getSelectionModel().select(selectedNode);//默认选择首节点
      					me.dictEntryId = selectedNode.data.id;
	    				me.tempGrid.store.proxy.url = me.tempGridqueryUrl;
	    				me.tempGrid.store.proxy.extraParams.dictTypeId = 'plan_temp_parameter';
	    				me.tempGrid.store.load();
						me.loadForm(me);
