...
//                me.code.setValue(responseJson.data.code);
//                me.processname.setValue(responseJson.data.name);
//                me.sort.setValue(responseJson.data.sort);
//                me.desc.setValue(responseJson.data.desc);
            	return true;
            }
        });
	    },
	   ...
	   save: function() {
    	...
    	if(processForm.isValid()) {
    		FHD.submit({
				...
				url : __ctxPath + '/process/process/saveframeprocess.f',
				callback: function (data) {
					if(!data.success){
						Ext.Msg.alert(FHD.locale.get('fhd.common.prompt'),data.info);
						return true;
					}else{
						if(me.processId.getValue()){
							
						}else{
							var tree = me.up('processmainpanel').processtree;
