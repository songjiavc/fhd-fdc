...
<body onload="isExistUser()">
<div id='div1' style="overflow:auto " >

<form:form commandName="empForm" action="${ctx}/sys/orgstructure/emp/edit.do" method="post">
	<input type="hidden" id=submitform name="submitform" value="baseInfo"/>
	...
	<table id="showTable" width="100%" border="0" cellpadding="0" cellspacing="0" class="fhd_form_table">
		...
		<table id="showUserTable" width="100%" border="0" cellpadding="0" cellspacing="0" class="fhd_form_table" style="display:none;">
			...
			<tr>
				...
				<form:input path="password" type="hidden" /></td>
				<th><spring:message code="fhd.common.status"/>：</th>
				<td>
				<form:select path="empStatus" name="empStatus" cssClass="required">
					<option value=""><spring:message code="fhd.common.pleaseSelect"/></option>
...
Ext.get('div1').setHeight(height);
//如果本员工有对应的用户就选中
function isExistUser(){
	if("${empForm.userid}".length>1){
		$("#checkOpr").attr("checked","checked");
		isDisplay();
	}
}
	function isDisplay() {	
...
<c:if test="${not empty success }">
	window.top.Ext.ux.Toast.msg('<spring:message code="fhd.common.operateSuccess" />', "<spring:message code='fhd.common.operateSuccess'/>");
</c:if>
