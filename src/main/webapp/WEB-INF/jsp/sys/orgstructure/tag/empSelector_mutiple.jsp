	...
	function handleSelect(node, show){
		...
		if(node.attributes.checked){
			if(node.isLeaf() && node.attributes.checked  && show.indexOf(nodeId)<0){
				var nodeDivStr="<div id='"+ nodeId +"'><input type='checkbox' checked='checked' onClick='javascript:removeCheckedValues(\"" + tags + "\",\"" + nodeId + "\")' name='" + tags + "' value='"+ nodeId +"' /><span ext:qtip='"+nodeText+"'>"+ nodeText + "</span></div>";
