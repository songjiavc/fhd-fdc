	...
	<table width="100%" height="550">
		<tr>
            <td rowspan="2" valign="top" width="200">
				...
				rootHref="${ctx}/components/org/empsEx.do?type=single&parentType=org&id=${orgRoot.id}" 
				
				el="tree-div" url="${ctx}/components/org/loadOrgTree.do?type=single&abName=${tag}" 
				...
				<div style="float:left;">
					<table class="selectorTable">
						...
						<tr bgcolor="#F7FDFD" >
							...
							<td width="575px" align="center" valign="middle">员工列表</td>
						</tr>
						...
						<tr bgcolor="#F7FDFD" ><td  height="40px" colspan="2" align="center" valign="middle">
							...
							<input type="button" value="取消" onclick="closeDialogWindow()" class="fhd_btn"/></td>
						</tr>
					</table>
				</div>
			</td>
		...
		function handleSelect(node, show){
			...
			if(node.attributes.checked){
				if(node.isLeaf() && node.attributes.checked  && show.indexOf(nodeId)<0){
					chooseValues.innerHTML ="<div id='"+ nodeId +"'><input type='checkbox' checked='checked' onClick='javascript:removeCheckedEmps(\"" + tags + "\",\"" + nodeId + "\")' name='" + abName + "' value='"+ nodeId +"' text = '"+nodeText+"' />"+ nodeText + "</div>";
				}
