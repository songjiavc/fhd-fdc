...
<head>
	...
	<script type="text/javascript">
				...
				<c:forEach items="${portal.subPortalPortlet }" var="portalPortlet">
					{
						...
						//autoLoad:'${ctx }${portalPortlet.portlet.url}',
		...
		function save(){
			...
			Ext.Ajax.request({
				type: "POST",
				url: "${ctx}/sys/portal/roleAssignPortal.do",
				params:'portalPortletsJson='+encodeURIComponent(Ext.encode(portalPortlets))+"&portalsJson="+encodeURIComponent(Ext.encode(portalsJson))+"&roleId="+$("#roleId").attr("value"),
				/*
				params:{
					portalPortletsJson: encodeURIComponent(Ext.encode(portalPortlets)),
					portalsJson: encodeURIComponent(Ext.encode(portalsJson)),
					roleId: $("#roleId").attr("value")
				},
				*/
				success: function(response) {
					window.top.Ext.ux.Toast.msg('提示', "保存成功!");
					closeWindow();
				},
				failure: function(response) {
					window.top.Ext.ux.Toast.msg('提示', "保存失败!");
					closeWindow();
				}
			});
		...
		Ext.onReady(function(){
			...
			portal.width=jQuery("#portal_div").width();
...
<body>
	...
	<div id="questSetDiv" style="height:23px; background:#f9f9f9;display: none;" >
